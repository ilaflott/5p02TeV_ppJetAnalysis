...
//finds the i'th bin inwhich a given value of eta fits in between the i'th and i+1'th xbin eta values
...
//finds the i'th bin inwhich a given value of pt fits in between the i'th and i+1'th xbin pt values
...
//returns missing Et from an event as a TLorentzVector
...
void deriveDijetResponse(int startfile = 0, int endfile = 1,
  ...
  for(int ifile = startfile; ifile<endfile; ++ifile){
	...
	if(phoEt->at(i)>30 && abs(phoEta->at(i))<1.4442){	  //cout << "photon found, Et: " << phoEt->at(i) << endl;
      ...
      if(nref>=2){ //need two or more jets
	int rJet, pJet;
