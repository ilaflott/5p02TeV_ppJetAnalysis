		 ...
		 std::string jetType = "PF")
{
  ...
  for(int nj=0; nj<knj; nj++){
	...
	cout<<"ip = "<<ip<<endl;
	if(ic < ncen)
	  hrsp[nj][ip]      = (TH1F*)fin->Get(Form("hJER_%d_pt_%d_cent%d",ptbins[ip], ptbins[ip+1],ic));
	
	if(ic == ncen)
	  hrsp[nj][ip]      = (TH1F*)finPP->Get(Form("hJER_%d_pt_%d",ptbins[ip], ptbins[ip+1]));
	...
	// if(nj==0 && ic==ncen-1){
	//   cout << "*******START*************** "<< ccent << " " << ptbins[ip]  << " to " << ptbins[ip+1] << endl;
	//   fitstatus = hrsp[nj][ip]->Fit(fgaus,"RLQ");
	//   cout <<" \t Fit Mean : " << fgaus->GetParameter(1) << " Hist Mean : " << mean << endl;
	//   cout << "*******END***************** "<< ccent << " " << ptbins[ip]  << " to " << ptbins[ip+1] << endl;
	//   cout << endl;
	// }
	// else fitstatus = hrsp[nj][ip]->Fit(fgaus,"RLQ");
	
	cout << " fitstatus : "<< fitstatus << " Fit Error : " << fgaus->GetParError(1) << " Hist Error " << hrsp[nj][ip]->GetMeanError() << endl;
