...
int JRsp(int wJetID=1, int radius = 3)
{
  int iSave=1;
  ...
  for(int nj=0; nj<knj; nj++){
    ...
    if(iSave){
      c11[nj]->SaveAs(Form("JEC_%d_PbPb_%s.gif",wJetID,(srad[nj]+plotAlgo).c_str()));
      c11[nj]->SaveAs(Form("JEC_%d_PbPb_%s.pdf",wJetID,(srad[nj]+plotAlgo).c_str()));
    }
  }
  //return 0;
  ...
  for(int nj=0;nj<knj;nj++){
	...
	if(ipad==1){
      ...
      if(iSave){
	c99[nj][ic]->SaveAs(Form("IndDist_PbPb_%s_%s.gif",ccent[ic],(srad[nj]+plotAlgo).c_str()));
	c99[nj][ic]->SaveAs(Form("IndDist_PbPb_%s_%s.pdf",ccent[ic],(srad[nj]+plotAlgo).c_str()));
      }
    }
  }
