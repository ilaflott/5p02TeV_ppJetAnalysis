...
int SVDUnfoldDataSpectra( std::string inFile_Data_dir, std::string inFile_MC_dir , const std::string baseName , 
  ...
  if(fillRespHists) hrec_sameside_resp_rebin = (TH1F*)hrec_sameside_rebin->Clone("recrebinsamesideClone4unf");
  else{    hrec_sameside_resp_rebin = new TH1F( ("hpp_rec_sameside_response_rebin"+RandEtaRange).c_str(),"", 
  ...
  if(drawPDFs && drawRespMatrix){    
    ...
    //RooUnfoldResponse roo_resp_ss(hrec_sameside_resp_rebin, hgen_resp_rebin, hmat_rebin, ("Response_matrix_rebin_ss"+radius).c_str());    
    //roo_resp.UseOverflow(doOverUnderflows);
    
    //RooUnfoldResponse roo_resp_ss_inv(0, 0, hmat_rebin, ("Response_matrix_rebin_nullresp_ss"+radius).c_str());    
    //RooUnfoldResponse roo_resp_ss_inv(hrec_sameside_resp_rebin_empty, hgen_resp_rebin_empty, hmat_rebin, ("Response_matrix_rebin_nullresp_ss"+radius).c_str());    
    //roo_resp_ss_inv.UseOverflow(doOverUnderflows);
    ...
    for(int kr = 0; kr < nKregMax; ++kr){
      ...
      //RooUnfoldResponse roo_resp( hmat_rebin, (roo_resp_title).c_str());    
      roo_resp.UseOverflow(doOverUnderflows);
      //roo_resp.Write();
      ...
      //hunf_ss_svd[kr]->SetMarkerStyle(kOpenCircle);
      ...
      hfold_ss_svd[kr] = (TH1F*)roo_resp.ApplyToTruth(hunf_ss_svd[kr]);      //hfold_ss_svd[kr] = (TH1F*)roo_resp_ss_inv.ApplyToTruth(hunf_ss_svd[kr]);
      hfold_ss_svd[kr]->Scale(hrec_sameside_resp_rebin->Integral()/hfold_ss_svd[kr]->Integral());
      //hfold_ss_svd[kr]->SetMarkerStyle(kOpenCircle);
	...
	c11->cd(1)->SetLogy();        //gPad->SetLogy();     
        ...
        hSVal->DrawCopy("HIST E");	//hSVal->Draw("E");
	...
	hdi->DrawCopy("HIST E"); //hdi->Draw("E");
	...
	c11_ss->cd(1)->SetLogy();        //gPad->SetLogy();     
        ...
        hSVal_ss->DrawCopy("HIST E");	//hSVal->Draw("E");
	...
	hdi_ss->DrawCopy("HIST E"); //hdi->Draw("E");
	...
	cPearsonMatrixIter->cd() ; cPearsonMatrixIter->Print(outPdfFile.c_str());  //cPearsonMatrixIter->Print(out3x3PdfFile.c_str());
	...
	//  cSpectra 3X3 and Check for meas --------------------------------------------------
	cSpectra->cd()           ;   cSpectra->Print(outPdfFile.c_str()); //cSpectra->Print(out3x3PdfFile.c_str());
	...
	leg2->AddEntry(hrec_unfolded_ratio[kRegDraw],"Data Unf./Data Meas.","pl");
	...
	cRatio_gen->cd()         ;   cRatio_gen->Print(outPdfFile.c_str());    //  cRatio_gen->Print(out3x3PdfFile.c_str());      
	...
	leg3->AddEntry(hgen_unfolded_ratio[kRegDraw],"MC Unf./MC Truth","pl");
	...
	//cRatioCheck_gen->Print(close_out3x3PdfFile.c_str());
      }	
