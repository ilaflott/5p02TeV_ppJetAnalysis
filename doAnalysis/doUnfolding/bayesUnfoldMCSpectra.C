...
int bayesUnfoldMCSpectra( std::string inFile_MC_dir , const std::string baseName , 
			  const bool doJetID=false , const bool useSimpBins=false){
  ...
  if(debugMode)std::cout<<"opening hist:"<<genHistTitle <<std::endl;
  ...
  if(debugMode)std::cout<<"opening hist:"<<TH2_title <<std::endl;
  ...
  // OPPSIDE UNFOLDING
  // ----------------------------------------------- //          
  if(debugMode)std::cout<<"unfold( O.S. Meas. Spectra)"<<std::endl;
  ...
  std::cout<<"fold(unfold ( O.S. Meas. Spectra ))"<<std::endl;
  TH1F* hfold=(TH1F*)roo_resp.ApplyToTruth(hunf);
  hfold->Scale(hrec_rebin->Integral()/hfold->Integral());//the unfolding correction has no guarantee of preserving the norm of the histo; neither does the inverse of the unfolding!  
  hfold->SetName("ppMC_BayesFold_Spectra");
  hfold->SetTitle("ppMC BayesFold Spectra");
  if(debugMode)hfold->Print("base");
  
  std::cout<<"fold( S.S. Truth Spectra ))"<<std::endl;
  TH1F* hfold_gen=(TH1F*)roo_resp.ApplyToTruth(hgen_rebin);
  //hfold_gen->Scale(hrec_rebin->Integral()/hfold_gen->Integral());  
  hfold_gen->SetName("ppMC_BayesFoldTruth_Spectra");
  hfold_gen->SetTitle("ppMC BayesFoldTruth Spectra");
  if(debugMode)hfold_gen->Print("base");
  ...
  // SAMESIDE UNFOLDING
  // ----------------------------------------------- //       
  std::cout<<"unfold ( S.S. Meas. Spectra )"<<std::endl;   
  ...
  // OPP SIDE RATIOS WITH SAME SIDE GEN
  TH1F *hgen_rebin_ratiobin=(TH1F*)hgen_rebin->Clone("ppMC_Gen_Ratio_denom");
  hgen_rebin_ratiobin=(TH1F*)hgen_rebin_ratiobin->Rebin(nbins_pt_reco,"ppMC_Gen_Ratio_denom_rebin",boundaries_pt_reco);
  if(debugMode)hgen_rebin_ratiobin->Print("base");
  
  TH1F *h_genratio_oppunf = (TH1F*)hunf->Clone( "ppMC_Gen_Ratio_OppUnf" );
  h_genratio_oppunf->SetTitle( "O.S. Unf./S.S. Truth" );
...
h_genratio_oppmeas->Divide(hgen_rebin_ratiobin);
  ...
  // OPP SIDE RATIOS WITH OPP SIDE MEAS
  ...
  h_recratio_oppunf->SetTitle( "O.S. Unf./O.S. Meas." );
  ...
  // SAME SIDE RATIOS WITH OPP SIDE MEAS
  ...
  h_recratio_ssunf->SetTitle( "S.S. Unf./O.S. Meas." );
  ...
  //TH1F *h_recratio_ssfoldgen = (TH1F*)hfold_gen->Clone( "ppMC_Meas_Ratio_SSFoldGen" );
  ////h_recratio_ssfoldgen=(TH1F*)h_recratio_ssfoldgen->Rebin(nbins_pt_reco, "ppMC_Meas_Ratio_SSFoldGen_rebin" , boundaries_pt_reco);
  //h_recratio_ssfoldgen->SetTitle( "S.S. Fold(Truth)/O.S. Meas." );
  //h_recratio_ssfoldgen->SetMarkerStyle(25);
  //h_recratio_ssfoldgen->SetMarkerColor(3);
  //h_recratio_ssfoldgen->Divide(hrec_rebin);
  ...
  h_recratio_ssgen->SetTitle( "S.S. Truth/O.S. Meas." );
  ...
  h_genratio_ssunf->SetTitle( "S.S. Unf./S.S. Truth" );
  ...
  //TH1F *h_genratio_ssfoldgen = (TH1F*)hfold_gen->Clone( "ppMC_Gen_Ratio_SSFoldGen" );
  ////h_recratio_ssfoldgen=(TH1F*)h_recratio_ssfoldgen->Rebin(nbins_pt_reco, "ppMC_Meas_Ratio_SSFoldGen_rebin" , boundaries_pt_reco);
  //h_genratio_ssfoldgen->SetTitle( "S.S. Fold(Truth)/S.S. Truth" );
  //h_genratio_ssfoldgen->SetMarkerStyle(25);
  ...
  h_genratio_ssmeas->SetTitle( "S.S. Meas./S.S. Truth" );
  ...
  //h_recratio_ssfoldgen->Write();  
  h_recratio_ssgen->Write();  
  
  covmat_TH2->Write("Covariance_Matrix_TH2D");
  PearsonBayes->Write("Pearson_Matrix_TH2D");
  ...
  if(!drawPDFs)std::cout<<std::endl<<"NOT drawing PDFs for Bayesian Unfolding!"<<std::endl<<std::endl;
    ...
    //if(useSimpBins)hgen_rebin->SetAxisRange(1.0e-16, 3.0e-4, "Y");
    //else hgen_rebin->SetAxisRange(9.0e-15, 3.0e-4, "Y");
    
    // ----------------------------------------------- //            
    // gen/meas/unf spectra hists ---------------
    // ----------------------------------------------- //            
    hgen_rebin->SetTitle("ppMC Spectra");
    ...
    TLegend* legend_genratio = new TLegend( 0.1,0.8,0.4,0.9 );
    //TLegend* legend_genratio = new TLegend( 0.5,0.75,0.9,0.9 );
    //    legend_genratio->AddEntry(h_genratio_oppunf, "O.S. Unf./S.S. Truth", "p");
    //    legend_genratio->AddEntry(h_genratio_oppmeas, "O.S. Meas./S.S. Truth", "p");
    //    //legend_genratio->AddEntry(h_genratio_oppfold, NULL, "p");
    //    legend_genratio->AddEntry(h_genratio_ssunf, "S.S. Unf./S.S. Truth", "p");
    //    legend_genratio->AddEntry(h_genratio_ssmeas, "S.S. Meas./S.S. Truth", "p");
    //    //legend_genratio->AddEntry(h_genratio_ssfold, NULL, "p");
    
    legend_genratio->AddEntry(h_genratio_oppunf,  "O.S. Unf./S.S. Truth", "p");//cause i change the title for the purposes of printing things earlier
    legend_genratio->AddEntry(h_genratio_oppmeas, NULL, "p");
    legend_genratio->AddEntry(h_genratio_ssunf,   NULL, "p");
    legend_genratio->AddEntry(h_genratio_ssmeas,  NULL, "p");
    legend_genratio->Draw();
