...
rooUnfoldCompile bayesUnfoldDataSpectra_wsmrPY8_etabin.C
...
THYDIR="smearTheory/PY8_01.05.21_sigmu_geny_noJERscales_spl3wgts_gaussSmear_semifinalv4_plots/"
THYFILESTR="PY8_01.05.21_sigmu_geny_noJERscales_spl3wgts_gaussSmear_semifinalv4_"
...
do
	...
	echo "${sysarr[i]}"
	echo "${sysarropt[i]}"
	./bayesUnfoldDataSpectra_wsmrPY8_etabin.exe ${DATADIR} ${OUTPUTTAG}_${sysarr[i]}sys${sysarropt[i]} $j ${THYDIR} ${THYFILESTR} ${dosystunf} ${sysarr[i]} ${sysarropt[i]} ${applyNPcorrs}
