...
std::string fNLO_location="/home/ilaflott/5p02TeV_ppJetAnalysis/CMSSW_7_5_8/src/doAnalysis/doUnfolding/smearTheory/fNLOJetsSpectra/R04/fnl5020_R04Jets_ybins_CT14nnlo_murEQmufEQpt_v3.root";
const int Nybins=4;
void reFitNPCs(){
  ...
  //TFile* finNLO=TFile::Open(fNLO_location.c_str(),"READ");
  //TH1* hNLO[Nybins]={};
  ...
  //TFormula* fitfunc=new TFormula( "NPfit", "[0]+[1]*pow(x,[2])");

  for(int i=0; i<Nybins; i++){
    
    data[i]=(TH1*)fin->Get(("h"+std::to_string(i)).c_str());
    
//    hNLO[i]=(TH1*)finNLO->Get(("h1100"+std::to_string(i+1)+"00").c_str());
//    float xmin=43.;
//    float xmax=-1.;
//    std::vector<double>ptbins;
//    for(int j=1;j<=hNLO[i]->GetNbinsX();j++){
//      if(hNLO[i]->GetBinContent(j)>0.){
//	xmax=hNLO[i]->GetBinLowEdge(j)+hNLO[i]->GetBinWidth(j);
//	ptbins.push_back(hNLO[i]->GetBinLowEdge(j));
//	std::cout<<"ptbins.at("<<j-1<<")="<<ptbins.at(j-1)<<std::endl;
//      }
//      else continue;
//    }
...
//    std::cout<<"ptbins.at("<<ptbins.size()-1<<")="<<ptbins.at(ptbins.size()-1)<<std::endl;
    ...
    ////this worked pretty good!
    //nufits[i]=new TF1(("nuf"+std::to_string(i)).c_str(),fitfunc.c_str(), xmin,220.);
    //nufits[i]->SetParameter(0,1.0);//should probably try initializing
    //nufits[i]->SetParameter(1,-0.1);//should probably try initializing
    //nufits[i]->SetParameter(2,-0.1);//should probably try initializing
    //std::cout<<"par0 init="<<nufits[i]->GetParameter(0)<<std::endl;
    //std::cout<<"par1 init="<<nufits[i]->GetParameter(1)<<std::endl;
    //std::cout<<"par2 init="<<nufits[i]->GetParameter(2)<<std::endl;
    //nufits[i]->SetParLimits(0,   0.5,2.);
    ...
    //std::string fitopt="M"  ;
    //std::string fitopt="E"  ;
    //std::string fitopt="RM" ;
    //std::string fitopt="ME" ;
    //std::string fitopt="RE" ;
    //std::string fitopt="MER";
    std::string fitopt="RE_MRE";
    //fitfailed=(bool)(data[i]->Fit(nufits[i]),fitopt.c_str());
    
    //fitfailed=(bool)(data[i]->Fit(nufits[i]),"RE");
    //fitfailed=(bool)(data[i]->Fit(nufits[i]),"MRE");
  ...
  for(int i=0; i<Nybins;i++){
    ...
    //nufits[i]->Draw("SAME");
    //data[i]->Draw("HIST E ][ SAME");
    //canv[i]->SaveAs( ("NLOpNP_InclusiveJets5TeV_fromJohn_refit_"+(std::string)canv[i]->GetName()+"_fitopt"+fitopt+"_NLOptbins.pdf").c_str());
    //canv[i]->SaveAs( ("NLOpNP_InclusiveJets5TeV_fromJohn_refit_"+(std::string)canv[i]->GetName()+"_fitopt"+fitopt+"_NPCptbins.pdf").c_str());
