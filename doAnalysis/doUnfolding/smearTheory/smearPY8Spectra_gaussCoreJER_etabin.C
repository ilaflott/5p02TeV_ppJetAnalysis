...
const int nEvents=1e+09;  ///new typical
...
//const std::string NPsys2_CorrFits_str =HERWGEE5_NPS;
...
int smearPY8Spectra_gaussCoreJER_etabin( std::string in_PY8fileString=in_PY8File_default, std::string fitType="modLog" , 
  ...
  std::string inputFile = SCRATCH_BASE+in_PY8fileString;//"fNLOJetsSpectra/R04/"+in_PY8fileString+".root";
  std::cout<<"opening input file:"<<inputFile<<std::endl<<std::endl;

  //TFile* fin_NLO=TFile::Open(inputFile.c_str());
  //std::string PDF_text=_PY8_DESCTXT;
  TFile* fin_PY8=TFile::Open(inputFile.c_str());
  std::string PY8_text=_PY8_DESCTXT;
  ...
  ////////// USE PDF SYS1/2 HISTS TO GET APPROPRIATE BINNING

  //v1 HISTS - using different PDFs to evaluate unc due to PDF choice    
  // another use; setting the binning on the toy PYTHIA8 correctly
  ...
  theory_PDFsys1->SetName( (thyname+"_PDFsys1").c_str() );    
  ...
  theory_PDFsys2->SetName( (thyname+"_PDFsys2").c_str() );    
  ...
  //  TH1D *theoryPDFNegerr = (TH1D*)fin_PY8->Get(thyPDFNegerrname.c_str());
  //  theoryPDFNegerr->SetTitle((PY8_text+" #sigma_{Jet} (-) PDF Errors;Jet p_{T} [GeV]; #||{(-) Errors} [%]").c_str());
  ...
  //  TH1D *theoryPDFPoserr = (TH1D*)fin_PY8->Get(thyPDFPoserrname.c_str());
  //  theoryPDFPoserr->SetTitle((PY8_text+" #sigma_{Jet} (+) PDF Errors;Jet p_{T} [GeV]; (+) Errors [%]").c_str());
  ...
  //  TH1D *theory2PtScaleNegerr = (TH1D*)fin_PY8->Get(thy2PtScaleNegerrname.c_str());
  //  theory2PtScaleNegerr->SetTitle((PY8_text+" #sigma_{Jet} (-) 2 Pt. Errors;Jet p_{T} [GeV]; #||{(-) Errors} [%]").c_str());
  ...
  //  TH1D *theory2PtScalePoserr = (TH1D*)fin_PY8->Get(thy2PtScalePoserrname.c_str());
  //  theory2PtScalePoserr->SetTitle((PY8_text+" #sigma_{Jet} (+) 2 Pt. Errors;Jet p_{T} [GeV]; (+) Errors [%]").c_str());
  ...
  //  TH1D *theory6PtScaleNegerr = (TH1D*)fin_PY8->Get(thy6PtScaleNegerrname.c_str());
  //  theory6PtScaleNegerr->SetTitle((PY8_text+" #sigma_{Jet} (-) 6 Pt. Errors;Jet p_{T} [GeV]; #||{(-) Errors} [%]").c_str());
  ...
  //  TH1D *theory6PtScalePoserr = (TH1D*)fin_PY8->Get(thy6PtScalePoserrname.c_str());
  //  theory6PtScalePoserr->SetTitle((PY8_text+" #sigma_{Jet} (+) 6 Pt. Errors;Jet p_{T} [GeV]; (+) Errors [%]").c_str());
  ...
  //assert(false);
  //remember, PY8 is done in bins of 1 GeV
  divideBinWidth(theory_rebin);
  divideBinWidth(theory_NP_rebin); 

  
  
  
  //why do i have this separate bin array again...?
  ...
  //
  //  if(i==n_thybins)
  //    thybins[n_thybins]=theory_rebin->GetBinLowEdge(i)+theory_rebin->GetBinWidth(i);
  //  else
  //    thybins[i]=theory_rebin->GetBinLowEdge(i+1);      
  //  
  //  std::cout<<"thybins["<<i<<"]="<<thybins[i]<<std::endl;
  //}
...
//  TH1D* theory_NP_PDFsysup_rebin=NULL, *theory_NP_PDFsysdown_rebin=NULL; //WITH NPCs
  if(doPDFsys){
    ...
    //    theory_PDFsysup->SetTitle((PY8_text+" sysup Inclusive #sigma_{jet}, "+absetarange_str+";Jet p_{T} [GeV];"+ddxsec_yax).c_str());
    ...
    //    theory_PDFsysdown->SetTitle((PY8_text+" sysdown Inclusive #sigma_{jet}, "+absetarange_str+";Jet p_{T} [GeV];"+ddxsec_yax).c_str());
  ...
  //else assert(false);//can't do np sysup/down for more forward bins yet; TO DO
    ...
    //    theory_NP_PDFsysup->SetTitle((PY8_text+"+"+NPCorrFits_text+" NPCs, sysup Inclusive #sigma_{jet}, "+absetarange_str+";Jet p_{T} [GeV];"+ddxsec_yax).c_str());
    ...
    //    theory_NP_PDFsysdown->SetTitle((PY8_text+"+"+NPCorrFits_text+" NPCs, sysdown Inclusive #sigma_{jet}, "+absetarange_str+";Jet p_{T} [GeV];"+ddxsec_yax).c_str());
  ...
  if(doNPsys){
    ...
    theory_NPsys1->SetTitle((PY8_text+"+"+NPsys1_CorrFits_text+" NPCs, Inclusive #sigma_{jet}, "+absetarange_str+";Jet p_{T} [GeV];"+ddxsec_yax).c_str());
    ...
    theory_NPsys2->SetTitle((PY8_text+"+"+NPsys2_CorrFits_text+" NPCs, Inclusive #sigma_{jet}, "+absetarange_str+";Jet p_{T} [GeV];"+ddxsec_yax).c_str());
...
//      theory_NPsys2_rebin=(TH1D*)theory_NPsys2->Clone(((std::string)theory_NPsys2->GetName()+"_rebin").c_str());    }
    ...
    theory_NPsysup->SetTitle((PY8_text+"+"+NPCorrFits_text+" sysup NPCs, Inclusive #sigma_{jet}, "+absetarange_str+";Jet p_{T} [GeV];"+ddxsec_yax).c_str());
    ...
    theory_NPsysdown->SetTitle((PY8_text+"+"+NPCorrFits_text+" sysdown NPCs, Inclusive #sigma_{jet}, "+absetarange_str+";Jet p_{T} [GeV];"+ddxsec_yax).c_str());
...
//      spline3_NP_PDFsysdown->SetLineColor(kAzure-8);      
    ...
    leg_spline->AddEntry(theory_rebin ,    ("Weighted "+PY8orderstring+" Jet Counts,"+absetarange_str           ).c_str() , "lp");
    leg_spline->AddEntry(spline3   ,       ("Cubic Spline "+PY8orderstring+","+absetarange_str                  ).c_str() , "l");
    //
    if(useSplineExt)leg_spline->AddEntry(spline3_ext  ,    ("Parabolic Spline Extension "+PY8orderstring+","+absetarange_str    ).c_str() , "l");
    //
    leg_spline->AddEntry(theory_NP_rebin , ("Weighted NP+"+PY8orderstring+" Jet Counts,"+absetarange_str        ).c_str() , "lp");
    leg_spline->AddEntry(spline3_NP   ,    ("Cubic Spline NP+"+PY8orderstring+","+absetarange_str               ).c_str() , "l");
    if(useSplineExt)leg_spline->AddEntry(spline3_NP_ext  , ("Parabolic Spline Extension NP+"+PY8orderstring+","+absetarange_str ).c_str() , "l");
    leg_spline->Draw();    
  }
  ...
  if(useFitWeights && useModLogFit){
    ...
    theory_rebin->SetTitle(("Modified Logarithm Fits for "+PY8orderstring+", "+absetarange_str+";Jet p_{T};Smear Weight").c_str());    
    theory_rebin->DrawClone("HIST E");    
    ...
    leg_logfits->AddEntry(theory_rebin , ("Weighted "+PY8orderstring+" Jet Counts for "+absetarange_str).c_str() , "lp");//change desc TO DO
    ...
    theory_NP_rebin->SetTitle( ("Modified Logarithm Fits for "+PY8orderstring+"+NP,"+absetarange_str+" ;Jet p_{T};Smear Weight").c_str());        
    theory_NP_rebin->DrawClone("HIST E");        
    ...
    leg_logfits_NP->AddEntry(theory_NP_rebin , ("Weighted "+PY8orderstring+"+NP Jet Counts for "+absetarange_str).c_str() , "lp");//change desc TO DO
  ...
  /// B) ddxsec[p_T] = [integral(ddxsec[p_T])] * [(p_T/GeV)^A] * [(1 - (2 * p_T / sqrt(s)]*cosh(ymin))^B] * [exp(-1*C/(p_T/GeV))]
    ...
    //draw
    std::cout<<"drawing fits and cross section on canvas"<<std::endl;
    theory_rebin->SetTitle(("7 TeV Fits for "+PY8orderstring+", "+absetarange_str+";Jet p_{T};Smear Weight").c_str());    
    theory_rebin->DrawClone("HIST E");        
    _7TeVFitA->Draw("SAME");
    _7TeVFitB->Draw("SAME");
    
    leg_7tevfits=new TLegend(0.65, 0.70, 0.9, 0.9, NULL,"BRNDC");    
    leg_7tevfits->AddEntry(theory_rebin , ("Weighted "+PY8orderstring+" Jet Counts for"+absetarange_str).c_str() , "lp");//change desc TO DO
    leg_7tevfits->AddEntry(_7TeVFitB   , ("4 Param. 7 TeV Fit for "+PY8orderstring+"").c_str() , "l");
    leg_7tevfits->AddEntry(_7TeVFitA   , ("3 Param. 7 TeV Fit for "+PY8orderstring+"").c_str() , "l");    
    leg_7tevfits->Draw();    
    ...
    //draw    
    std::cout<<"drawing NP fits and cross section on canvas"<<std::endl;
    theory_NP_rebin->SetTitle( ("7 TeV Fits for NP+"+PY8orderstring+","+absetarange_str+";Jet p_{T};Smear Weight").c_str());    
    theory_NP_rebin->DrawClone("HIST E");    
    ...
    leg_7tevfits_NP->AddEntry(theory_NP_rebin , ("Weighted NP+"+PY8orderstring+" Jet Counts for"+absetarange_str).c_str() , "lp");//change desc TO DO
    leg_7tevfits_NP->AddEntry(_7TeVFitA_NP   ,  ("3 Param. 7 TeV Fit for NP+"+PY8orderstring).c_str() , "l");
    leg_7tevfits_NP->AddEntry(_7TeVFitB_NP   ,  ("4 Param. 7 TeV Fit for NP+"+PY8orderstring).c_str() , "l");    
  ...
  TH1D *theory_rnd_NP       = (TH1D*)theory_rnd->Clone("theory_rnd_NP"      );  
  TH1D *smeared_rnd_NP      = (TH1D*)smeared_rnd->Clone("smeared_rnd_NP"     );
  TH1D *smeared_rnd_NP_test = (TH1D*)smeared_rnd_test->Clone("smeared_rnd_NP_test");
  TH2D* response_NP_th2     = (TH2D*)response_th2->Clone("response_NP_th2"    );
  ...
  theory_rnd->SetTitle( ((std::string)("Toy MC Incl. "+PY8orderstring+" #sigma;;"+ddxsec_yax)).c_str() );
  ...
  leg_smear->AddEntry(theory_rnd  ,      ("Truth Incl. "+PY8orderstring+" #sigma"            ).c_str(), "lp" );
  leg_smear->AddEntry(smeared_rnd ,      ("Smeared Incl. "+PY8orderstring+" #sigma"          ).c_str(), "lp" );
  leg_smear->AddEntry(smeared_rnd_test , ("Opp Side Smeared Incl. "+PY8orderstring+" #sigma" ).c_str(), "lp" );
  leg_smear->Draw();  
  ...
  response_th2->SetTitle(("Toy MC Response Matrix from JER Smeared "+PY8orderstring+" #sigma_{jet}").c_str());  
  ...
  if(doJERsys){
    ...
    //		      (TH2D*)response_JERsysup_th2); }  
     ...
     theory_rnd_NP_JERsysup      = (TH1D*)  theory_rnd_JERsysup->Clone(      "theory_rnd_NP_JERsysup"      );  
    smeared_rnd_NP_JERsysup      = (TH1D*) smeared_rnd_JERsysup->Clone(     "smeared_rnd_NP_JERsysup"     );
    smeared_rnd_NP_JERsysup_test = (TH1D*) smeared_rnd_JERsysup_test->Clone("smeared_rnd_NP_JERsysup_test");
       response_NP_JERsysup_th2  = (TH2D*)response_JERsysup_th2->Clone(        "response_NP_JERsysup_th2"    );
...
//		      (TH2D*)response_JERsysdown_th2); }  
     ...
     theory_rnd_NP_JERsysdown      = (TH1D*)  theory_rnd_JERsysdown->Clone(      "theory_rnd_NP_JERsysdown"      );  
    smeared_rnd_NP_JERsysdown      = (TH1D*) smeared_rnd_JERsysdown->Clone(     "smeared_rnd_NP_JERsysdown"     );
    smeared_rnd_NP_JERsysdown_test = (TH1D*) smeared_rnd_JERsysdown_test->Clone("smeared_rnd_NP_JERsysdown_test");
       response_NP_JERsysdown_th2  = (TH2D*)response_JERsysdown_th2->Clone(        "response_NP_JERsysdown_th2"    );
...
//
  ...
  //-----------------------------------------------------------------------------------------//
  std::cout<<"creating TH1 for toy NP+"+PY8orderstring+" spectra generation"<<std::endl<<std::endl;
...
//				     (Int_t)n_thybins, (Double_t*)thybins);
  ...
  theory_rnd_NP->SetTitle( ((std::string)("Toy MC Incl. NP+"+PY8orderstring+" #sigma;;"+ddxsec_yax)).c_str() );
  ...
  NPleg_smear->AddEntry(theory_rnd_NP  , ("Truth Incl. NP+"+PY8orderstring+" #sigma"   ).c_str(), "lp" );
  NPleg_smear->AddEntry(smeared_rnd_NP , ("Smeared Incl. NP+"+PY8orderstring+" #sigma" ).c_str(), "lp" );
  NPleg_smear->AddEntry(smeared_rnd_NP_test , ("Opp Side Smeared Incl. NP+"+PY8orderstring+" #sigma" ).c_str(), "lp" );
  NPleg_smear->Draw();  
      
  // ratio pad
  NPratio_pad->cd();
  
  std::cout<<"creating ratio hists for NP+"+PY8orderstring+""<<std::endl;
  TH1D *NP_true_smeared_rat=(TH1D*)theory_rnd_NP->Clone("NP_true_smeared_rat"); 
  if(printBaseDebug)NP_true_smeared_rat->Print("base");  
  for(int i=1; i<=NP_true_smeared_rat->GetNbinsX(); i++) 
    std::cout<<"NP_true_smeared_rat->GetBinLowEdge("<<i<<")="<<NP_true_smeared_rat->GetBinLowEdge(i)<<std::endl;
  ...
  response_NP_th2->SetTitle(("Toy MC Response Matrix from JER Smeared NP+"+PY8orderstring+" #sigma_{jet}").c_str());
...
//						      );
  ...
  std::cout<<""+PY8orderstring+" THY/RND THY LAST BIN CONTENT = "<<theory_ratio_rnd->GetBinContent(theory_ratio_rnd->GetNbinsX()) <<std::endl;
  std::cout<<""+PY8orderstring+" THY/RND THY LAST BIN CONTENT = "<<theory_NP_ratio_rnd_NP->GetBinContent(theory_NP_ratio_rnd_NP->GetNbinsX()) <<std::endl;
  ...
  leg_tru_rnd_rat->AddEntry( theory_ratio_rnd,       (PY8orderstring+" Spectra"   ).c_str(), "lp");
  leg_tru_rnd_rat->AddEntry( theory_NP_ratio_rnd_NP, (PY8orderstring+"+NP Spectra").c_str(), "lp");
...
//    response_NP_PDFsysdown_th2->Write();  
  ...
  Int_t realmin=(Int_t)timer.RealTime()/60;
  Float_t realmin_rem=timer.RealTime()/60. - ((Float_t)(realmin));
  Int_t realmin_rem_sec= (Int_t)(realmin_rem*60.)/1;

  Int_t cpumin=(Int_t)timer.CpuTime()/60;
  Float_t cpumin_rem=timer.CpuTime()/60. - ((Float_t)(cpumin));
  Int_t cpumin_rem_sec= (Int_t)(cpumin_rem*60.)/1;

  std::cout<<"CPU   time  = " << cpumin  << " min " << cpumin_rem_sec   << " sec" << std::endl;
  std::cout<<"Real  time  = " << realmin << " min " << realmin_rem_sec  << " sec" << std::endl;
...
int main(int argc, char* argv[]){  
  ...
  if(argc==6){
    rStatus=smearPY8Spectra_gaussCoreJER_etabin( 
						   (std::string) argv[1] ,  
						   (std::string) argv[2], 
						   (std::string) argv[3] ,
						   (int)std::atoi(argv[4]) ,
