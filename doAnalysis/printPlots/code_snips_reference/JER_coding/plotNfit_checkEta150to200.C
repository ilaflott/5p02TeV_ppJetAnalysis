...
int plotNfit(const std::string inFile_MC_dir,const std::string baseName){
  ...
  // input hists
  TH1F *hrsp[Nrad][nbins_eta]={};  
  double array_mean[Nrad][nbins_eta];
  double array_sig[Nrad][nbins_eta];
  ...
  for(int nj=0; nj<Nrad; nj++){
    ...
    hMean[nj]->SetAxisRange(0.8,1.2, "Y");
    hMean[nj]->SetAxisRange(-5.3,5.3, "X");
    ...
    hSigma[nj]->SetAxisRange(-5.3,5.3, "X");
    ...
    for(int ip=0; ip<nbins_eta; ip++){
      ...
      if(hrsp[nj][ip]->GetEntries()<5){//not enough entries
  ...
  { 
	...
	std::string hrspTitle="JER, 150<genpt<200, "+std::to_string(etabins[j])+"<geneta<"+std::to_string(etabins[j+1]);
