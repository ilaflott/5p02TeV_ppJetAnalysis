...
//  std::string etabin = "eta32to47";
  ...
  fMC = TFile::Open("/export/d00/scratch/ilaflott/5p02TeV_ppJetAnalysis/readForests/1.16.17_outputCondor/ppMC_Py8_CUETP8M1_QCDjetAllPtBins_ak4PFJets_01-16-17_jetPlots_jtIDv3L_eta20/Py8_CUETP8M1_QCDjetAllPtBins_ak4PF-allFiles.root");
std::cout<<"loaded file";std::cout<<endl;
  ...
  for(int i = 0; i<N; ++i){ 
    ...
    for(int k = 0; k<2; ++k){
      ...
      hJetQA[k][0][i] = (TH1F*)fMC->Get(Form("hJetQA_%dwJetID_%s",k,var[i].c_str()));
...
//    cJetQA[i]->SaveAs(Form("%s/JetQAPlots_pp_5TeV_%s.pdf", etabin.c_str(),var[i].c_str()),"RECREATE");    
    cJetQA[i]->SaveAs(Form("JetIDEff_ppMC_Py8_CUETP8M1_QCDjetAllPtBins_ak4PFJets_eta20_01-17-17_jetPlots_jtIDv3_5TeV_%s.pdf", var[i].c_str()),"RECREATE");    //check radius and eta
  }// var loop
