...
const bool drawDataMCOverlaysInput=false; 		 //this should always be false, but needs to be cleaned up later.
  ...
  if(drawDataMCOverlaysInput)thePDFFileName+="_"+output_PDFname_base+"_Overlaid_test.pdf";
  else thePDFFileName+="_"+output_PDFname_base+"_Ratios_test.pdf";//one of these two will ALWAYS fire
  ...
  //else if(doJetIDPlots)std::cout<<std::endl<<"doing JetID plots, skipping evt QA plots..."<<std::endl<<std::endl;
    ...
    TCanvas *temp_canvEvt_wLog = new TCanvas("tempEvt", "temp Evt Canv", 1000, 600);//This is used for the Jet QA Evt plots with log plots.
    ...
    if(debugMode)std::cout<<"creating temporary canvas for printing Evt plots..."<<std::endl;
    TCanvas *temp_canvEvt = new TCanvas("tempEvt_wLog", "temp Evt Canv withLog", 800, 800); //This is for the Jet QA Evt plots without logs
    temp_canvEvt->cd();
	TPad *evtpad1 = new TPad("evtpad1", "Overlay Pad",0.0,0.3,1.0,1.0);
...
//      if(drawDataMCOverlays){  
	...
	else if(var[j]=="leadJetPt"||var[j]=="subleadJetPt"){ temp_canvJet->cd(0);
...
int main(int argc, char *argv[]){
  ...
  if( argc!=4 ) {//no input arguments, error
    std::cout<<"do ./printPlots_jtEvtHists.exe <ppDataCondorDir> <ppMCCondorDir> <outputNameBase>"<<std::endl;
    return rStatus;
  }
  rStatus=1;
  
  //if(argc==1)rStatus=printPlots_jtEvtHists();
  if(argc==4) rStatus=printPlots_jtEvtHists( (const std::string) argv[1], (const std::string) argv[2],
