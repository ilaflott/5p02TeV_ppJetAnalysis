...
//int printPlots_HINvSMPjetIDPlots(const std::string input_condorDir ,
int printPlots_HINvSMPjetIDPlots(const std::string input_condorDir_HINjetID , 
				 const std::string input_condorDir_SMPjetID , 
				 const std::string output_PDFname_base ){
  
  
  
  //figure out what radius/jetcollection we are looking at using the ppData filename
  std::size_t radPos=input_condorDir_HINjetID.find("_ak")+3;  
  const std::string radiusInt= input_condorDir_HINjetID.substr( radPos,1 );
  ...
  std::size_t jetsPos=input_condorDir_HINjetID.find("Jets");;
  
  //make final jetType strings
  const std::string jetType=input_condorDir_HINjetID.substr( jetTypePos,(jetsPos-jetTypePos) );
  ...
  const std::string fullFilename_HIN_jetID=inputDir+input_condorDir_HINjetID+input_Filename;
  const std::string fullFilename_SMP_jetID=inputDir+input_condorDir_SMPjetID+input_Filename;
  
  // OPEN INPUT SECTION
  TFile *fin_HINjetID=NULL;
  TFile *fin_SMPjetID=NULL;
  ...
  std::cout<<" in directory: "<<inputDir+input_condorDir_HINjetID<<std::endl<<std::endl;
  fin_HINjetID = new TFile(fullFilename_HIN_jetID.c_str());      
  
  std::cout<<" now opening SMP Jet ID file "<<input_Filename<<std::endl;//fullFilename_jetID<<std::endl;
  std::cout<<" in directory: "<<inputDir+input_condorDir_SMPjetID<<std::endl<<std::endl;
  fin_SMPjetID = new TFile(fullFilename_SMP_jetID.c_str());      
  ...
  theROOTFileName+="_"+output_PDFname_base+"_HINvSMPjetIDPlots.root";
  TFile* fout=new TFile(theROOTFileName.c_str(), "RECREATE");
  
  std::string thePDFFileName=outputDir+fullJetType;
  thePDFFileName+="_"+output_PDFname_base+"_HINvSMPjetIDPlots.pdf";
  ...
  std::cout<<" drawing HINvSMP jetID Plots..."<<std::endl;

  
  for( int etabin=0; etabin<Netabins ; etabin++ ){
    std::cout<<std::endl;
    printHINvSMPJetIDHist_pt( (TFile*) fin_HINjetID , (TFile*) fin_SMPjetID , 
...
int main(int argc, char *argv[]){
  ...
  if( argc!=4 ) {//no input arguments, error
    std::cout<<"do ./printPlots_HINvSMPjetIDPlots.exe <inputCondorDir> <outputNameBase>"<<std::endl;
    return rStatus;
  }
  rStatus=1;
  
  if(argc==4) rStatus=printPlots_HINvSMPjetIDPlots( (const std::string) argv[1], (const std::string) argv[2] , (const std::string) argv[3] );
