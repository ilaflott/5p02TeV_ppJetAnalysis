...
const bool draw_hJERRapBins=true, doGenBinsToo=false;//RapBins -> dual-diff xsec bins, GenBins -> variable, depends on readForests
const bool draw_JERgen150to200=true, draw_JERgen30to50=true;
...
int printPlots_MCJEC(const std::string inFile_MC_dir,const std::string outputTag){
  ...
  //JER across eta bins
  // input hists
  if(!draw_hJER)std::cout<<std::endl<<std::endl<<"skipping hJER hists"<<std::endl<<std::endl;
...
// initialize mean hist    
    ...
    hMean[nj]->SetAxisRange(0.9,1.3, "Y");    
  ...
  {         
    ...
    for(int i=0;i<Nrad;++i){    
      ...
      hMean[i] ->Draw("eL"); //pdfoutCanv->Print(thePDFFileName.c_str());    
      TLine* meanLine=new TLine(10,1.,500,1.);    
  ...
  for(int nj=0; nj<Nrad; nj++){    
    ...
    hSigma[nj]->SetLineColor(1);    
    hSigma[nj]->SetMarkerSize(1.0);    
    MakeHistRMSEta(hSigma[nj],0.563,0.001);     
    hSigma[nj]->SetAxisRange(0.0,0.5, "Y");    
    //hSigma[nj]->SetAxisRange(0.,5., "X");    
  ...
  {         
    // DRAW THOSE PDFS //    
    std::string thePDFFileName=outputDir+fullJetType+jobType+"_"+outputTag+"_RapBins.pdf";    
    std::string open_thePDFFileName=thePDFFileName+"[";    
    std::string close_thePDFFileName=thePDFFileName+"]";    
    ...
    for(int i=0;i<Nrad;++i){    
      p1->cd();      
      hMean[i] ->Draw("L e"); //pdfoutCanv->Print(thePDFFileName.c_str());    
      TLine* meanLine=new TLine(0.,1.,4.7,1.);    
      meanLine->SetLineStyle(2);       meanLine->SetLineColor(kBlue);    
      meanLine->Draw();    
	...
	std::string hrspTitle="hJER_"+std::to_string(absetabins[j])+"_pt_"+std::to_string(absetabins[j+1]);    
	hrsp[i][j]->SetTitle(hrspTitle.c_str());    
	hrsp[i][j]->SetMarkerStyle(8);    
	hrsp[i][j]->SetMarkerSize(1.0);    
	hrsp[i][j]->GetXaxis()->SetTitle("recpt/genpt");    
...
(pow(-hrsp_gen150[nj][ip]->GetStdDev()/pow(mean,2), 2) * pow(emean,2)) ); 
  ...
  //if(debugMode)std::cout<<"jobType="<<jobType<<std::endl;
  { 
    std::string thePDFFileName=outputDir+fullJetType+jobType+"_"+outputTag+"_gen150to200.pdf";
    std::string open_thePDFFileName=thePDFFileName+"[";
    std::string close_thePDFFileName=thePDFFileName+"]";
    ...
    for(int i=0;i<Nrad;++i){
      pdfoutCanv->cd(1);
      hMean_gen150[i] ->Draw("eL"); //pdfoutCanv->Print(thePDFFileName.c_str());
      TLine* meanLine=new TLine(-5.3,1.,5.3,1.);
      meanLine->SetLineStyle(2); meanLine->SetLineColor(kBlue);
      meanLine->Draw();

      pdfoutCanv->cd(2);
      hSigma_gen150[i]->Draw("eL"); 
...
continue;      }
  ...
  //if(debugMode)std::cout<<"jobType="<<jobType<<std::endl;
  { 
    std::string thePDFFileName=outputDir+fullJetType+jobType+"_"+outputTag+"_gen30to50.pdf";
    std::string open_thePDFFileName=thePDFFileName+"[";
    std::string close_thePDFFileName=thePDFFileName+"]";
    ...
    for(int i=0;i<Nrad;++i){
      pdfoutCanv->cd(1);
      hMean_gen30[i] ->Draw("eL"); //pdfoutCanv->Print(thePDFFileName.c_str());
      TLine* meanLine=new TLine(-5.3,1.,5.3,1.);
      meanLine->SetLineStyle(2);      meanLine->SetLineColor(kBlue);
      meanLine->Draw();

      pdfoutCanv->cd(2);
      hSigma_gen30[i]->Draw("eL"); 
      //TLine* sigmaLine=new TLine(-5.3,0.1,5.3,0.1);
      //sigmaLine->Draw();
      pdfoutCanv->Print(thePDFFileName.c_str());    
