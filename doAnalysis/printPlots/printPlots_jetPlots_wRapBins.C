...
const bool drawJetConstituentPlots=false, drawDijetPlots=false;//, doBasicJetQAOnly=true;
const bool drawJetTrigQAPlots=false, drawJetRapBinsPlot=true ;
...
const int theRapOverlayMarkerColor[] ={  2,  3,  6,  7,  4,  9 , 8, 1};
// markers
...
int printPlots_jtEvtHists(const std::string input_ppData_condorDir , const std::string input_ppMC_condorDir , 
  ...
  if(!doEventCounts)std::cout<<"skipping evt/jet QA counts + plots..."<<std::endl<<std::endl;
	...
	std::cout<<"total integrate xsec for jets = " << h_NJets->GetEntries()/(effIntgrtdLumi_vz) << std::endl;
  ...
  if( !drawJetRapBinsPlot || drawDataMCRatios )
    ...
    TCanvas *temp_canvJetRapBins = new TCanvas("tempJetRapBins", "tempJetRapBins", 1500, 900);
	...
	if(debugMode)std::cout<<"inHistName="<<inHistName<<std::endl;	
	...
	float integral=theJetSpectraRapHist->Integral();
	float binWidth=theJetSpectraRapHist->GetBinWidth(1);
	if(rapbin==7) binWidth=binWidth*3.;
	std::cout<<"binWidth="<<binWidth<<std::endl;
	std::cout<<"integral="<<integral/power*binWidth<<std::endl;
	totalIntXsec=totalIntXsec+binWidth*(integral/power);
	std::cout<<"totalIntXsec="<<totalIntXsec<<std::endl;


	theJetSpectraRapHist->SetMarkerStyle( kFullDotLarge ) ;//theRapOverlayMarker[j]);
	theJetSpectraRapHist->SetMarkerSize(0.98);
	...
	if(rapbin==0){
	  ...
	  theJetSpectraRapHist->SetAxisRange(std::pow(10.,-7.),std::pow(10.,6.5),"Y");
      ...
      std::cout<<"TOTAL INT JET XSEC="<<""<<std::endl;
  ...
  if(!drawJetTrigQAPlots)
    ...
    TCanvas *temp_canvJetTrig = new TCanvas("tempJetTrig", "tempJetTrig", 1300, 900);
    temp_canvJetTrig->cd();
    temp_canvJetTrig->SetLogy(1);    
    temp_canvJetTrig->SetLogx(1);     
    //temp_canvJetTrig->SetGridx(1);
    //temp_canvJetTrig->SetGridy1(1);
   
    if(debugMode)std::cout<<std::endl<<"creating temporary canvas for printing JetTrig plots..."<<std::endl;
    TCanvas *temp_canvJetRatio = new TCanvas("tempJetRatio", "tempJetRatio", 1300, 600);
    temp_canvJetRatio->cd();
    temp_canvJetRatio->SetLogy(0);
    temp_canvJetRatio->SetLogx(1);
    //temp_canvJetRatio->SetGridx(1);
    //temp_canvJetRatio->SetGridy1(1);
    

    
    if(drawTrigDataOverlays) { temp_canvJetTrig->cd();
      
      TLegend* JetTrigLegend=new TLegend(0.75,0.66,0.88,0.86, NULL,"brNDC");
	...
	if(j==0){
      ...
      TLegend* JetTrigRatioLegend=new TLegend(0.12,0.64,0.30,0.87, NULL,"brNDC");    
