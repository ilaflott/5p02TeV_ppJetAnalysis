...
int jetWeights( std::string infile_Forest , std::string outputWeightFile ){
  ...
  // declare chain + tree we're interested in
  TChain* nt;
  nt = new TChain("ak4PFJetAnalyzer/t");
  // TChain* evt;
  //evt = new TChain("hiEvtAnalyzer/HiTree");

  // open filelist + add to TChain until reaching end of filelist.
  std::cout<<"adding files to TChain..."<<std::endl;
  std::ifstream instr_Forest(infile_Forest.c_str(),std::ifstream::in);
  bool endOfFilelist=false; int fileCount=0;
  while(!endOfFilelist){
    std::string filename_Forest;
    instr_Forest>>filename_Forest;
    if(filename_Forest==""){std::cout<<"end of filelist!"<<std::endl; endOfFilelist=true; break;}//end of filelist condition
    std::cout<<"adding file #"<<fileCount<<",  "<<filename_Forest<<std::endl;
    fileCount++;
    nt->AddFile(filename_Forest.c_str());
  }
  assert(endOfFilelist);//otherwise why bother, make sure i'm hitting all the files

  std::cout<<"all files loaded, counting MC events in pthat bins..."<<std::endl;
  int n[NpthatBins];
  ...
  for(int i = 0; i < NpthatBins; ++i){
    ...
    float pthatweight;
    double xsDiff=xs2015[i]-xs2015[i+1];
    if(n[i]) pthatweight=(float)(xsDiff/n[i]);
    else pthatweight=-1 ;
    std::cout<<"the weight is "<<pthatweight<<" mb^-3"<<std::endl;
    outputFile<<"n["<<i<<"]="<<n[i]<<", ";  
