// custom header
#include "readForests_ppData_HighPtLowerJets.h"

const double jtPtCut=15.,jtEtaCut=2.;//JETCUTS
const double ldJetPtCut=60., subldJetPtCut=30., jetQAPtCut=50.;//DIJET CUTS, QA CUT

//// readForests_ppData_HighPtLowerJets
// ---------------------------------------------------------------------------------------------------------------
int readForests_ppData_HighPtLowerJets(int startfile , int endfile , 
				       std::string inFilelist , std::string outfile ,
  ...
  TH1F *h_NEvents_trgDup     = new TH1F("NEvents_trgDup"    , "NEvents post evt cuts, duplicate count ", 1,0,2);
  TH1F *h_NEvents_trgDupCut  = new TH1F("NEvents_trgDupCut" , "NEvents post evt cuts, non-duplicates", 1,0,2);
  ...
  for(UInt_t nEvt = 0; nEvt < NEvents_read; ++nEvt) {//event loop   
    ...
    //duplicate skipping: any jet80/100 event has already been read in HighPtJet80 filelist
    if( (trgDec[0]||trgDec[1]) && (trgDec[2]||trgDec[3]) ) {
      if(debugMode)for(int k=0;k<4;++k)if(trgDec[k])std::cout<<"trgDec["<<k<<"]="<<trgDec[k]<<std::endl;
      if(debugMode)std::cout<<"skipping jet80/jet100 event"<<std::endl;
      h_NEvents_trgDup->Fill(1);
      continue;    }
    h_NEvents_trgDupCut->Fill(1);
  ...
  std::cout<<std::endl;
  std::cout<<"Total Num of duplicate Events read post Evt. Cuts = " <<
    h_NEvents_trgDup->GetEntries()<<std::endl;
  std::cout<<"Total Num of Events read post trgDup Cut = " <<
  ...
  std::cout<<std::endl<<"readForests_ppData_HighPtLowerJets finished."<<std::endl;  timer.Stop();
  std::cout<<"CPU time (min)  = "<<(Float_t)timer.CpuTime()/60<<std::endl;
  std::cout<<"Real time (min) = "<<(Float_t)timer.RealTime()/60<<std::endl;

  return 0;
}// end readForests_ppData_HighPtLowerJets
...
int main(int argc, char *argv[]){
  ...
  if(argc!=8 && argc!=1){
    ...
    std::cout<<"./readForests_ppData_HighPtLowerJets.exe";
    std::cout<<std::endl<<std::endl;
  ...
  // good input, run
  rStatus=1;
  if(argc==1) rStatus = readForests_ppData_HighPtLowerJets();
  else {//read input argument vector
    int startfile= atoi(argv[1]), endfile= atoi(argv[2]); 
    std::string inputFileList=argv[3], outputFileName=argv[4];
    int jetRadius= atoi(argv[5]); std::string jetType=argv[6];     
    bool debug=(bool)atoi(argv[7]);          
    rStatus = readForests_ppData_HighPtLowerJets( startfile, endfile, 
