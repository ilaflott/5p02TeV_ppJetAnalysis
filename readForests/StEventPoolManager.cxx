#ifndef StEventPoolManager_cxx
#define StEventPoolManager_cxx
...
Long64_t StEventPool::Merge(TCollection* hlist)
{
  ...
  while ( (tmpObj = static_cast<StEventPool*>(objIter())) )
  {
    ...
    for(Int_t i=0; i<((int)tmpObj->fEvents.size()); i++)
...
void StEventPoolManager::Validate()
{
  ...
  for(Int_t i = 0; i<((int)fMultBins.size()); i++)
    tmpStr += Form("%4.3f ", fMultBins[i]);
  tmpStr += " vertex: ";
  for(Int_t i = 0; i<((int)fZvtxBins.size()); i++)
    tmpStr += Form("%4.3f ", fZvtxBins[i]);
  tmpStr += " psi: ";
  for(Int_t i = 0; i<((int)fPsiBins.size()); i++)
    tmpStr += Form("%4.3f ", fPsiBins[i]);
  tmpStr += " pt: ";
  for(Int_t i = 0; i<((int)fPtBins.size()); i++)
