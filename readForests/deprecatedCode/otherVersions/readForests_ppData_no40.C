// custom header
#include "readForests_ppData_no40.h"
...
// ---------------------------------------------------------------------------------------------------------------
int readForests_ppData_no40(int startfile , int endfile , std::string inFilelist , std::string outfile ,
  ...
  jetpp[3]->SetBranchAddress( HLTBranches[0].c_str() , &jet60_I);
  jetpp[3]->SetBranchAddress( HLTBranches[1].c_str() , &jet80_I);
  jetpp[3]->SetBranchAddress( HLTBranches[2].c_str() , &jet100_I);
  
  if(debugMode)for(int i=0;i<N_L1Bits;i++)
    std::cout<<"HLTPresclBranches[i="<<i<<"]="<<HLTPresclBranches[i]<<std::endl;
  //  jetpp[3]->SetBranchAddress( HLTPresclBranches[0].c_str() , &jet40_p_I);
  jetpp[3]->SetBranchAddress( HLTPresclBranches[0].c_str() , &jet60_p_I);
  jetpp[3]->SetBranchAddress( HLTPresclBranches[1].c_str() , &jet80_p_I);
  jetpp[3]->SetBranchAddress( HLTPresclBranches[2].c_str() , &jet100_p_I);
  ...
  jetpp[3]->SetBranchAddress( L1PresclBranches[0].c_str()  , &jet60_l1s_ps_I);
  jetpp[3]->SetBranchAddress( L1PresclBranches[1].c_str()  , &jet80_l1s_ps_I);
  jetpp[3]->SetBranchAddress( L1PresclBranches[2].c_str()  , &jet100_l1s_ps_I);
  ...
  jetpp[4]->SetBranchAddress("pt",&trgObjpt_60);  
  jetpp[5]->SetBranchAddress("pt",&trgObjpt_80);  
  jetpp[6]->SetBranchAddress("pt",&trgObjpt_100);
...
//			       Form("Spectra from Jet 40 && !jet60 && !jet80 R%d %s" , radius,etaWidth), 2000,0,2000);
  ...
  assert( jetEventsExist && skimEventsExist 
	  && EventCountsEqual );//exit if the counts don't make sense, we don't want to segfault
  ...
  for(UInt_t nEvt = 0; nEvt < NEvents_read; ++nEvt) {//event loop   
    ...
    int treePrescl[N_L1Bits]={ /*(jet40_p_I*jet40_l1s_ps_I),*/ (jet60_p_I*jet60_l1s_ps_I), 
    ...
    bool trgDec[N_L1Bits]={ /*(bool)jet40_I,*/ (bool)jet60_I, //HLT decision=HLT&&L1=HLT&&true
		     (bool)jet80_I, (bool)jet100_I  }; 
    ...
    if( trgDec[0] && triggerPt>=60.  && triggerPt<80.  ) is60  = true;    
    if( trgDec[1] && triggerPt>=80.  && triggerPt<100. ) is80  = true;    
    if( trgDec[2] && triggerPt>=100.                   ) is100 = true;    
	...
	hNEvts_withJets_Filled=true;      }      
      ...
      if(is60)  hpp_TrgObj60[0]->Fill(recpt, treePrescl[0]);
      if(is80)  hpp_TrgObj80[0]->Fill(recpt, treePrescl[1]);
      if(is100) hpp_TrgObj100[0]->Fill(recpt, treePrescl[2]);
	...
	hNEvts_withJets_kmatCut_Filled=true;      }
      ...
      if(is60 )  hpp_TrgObj60[1]->Fill(recpt, treePrescl[0]);
      if(is80 )  hpp_TrgObj80[1]->Fill(recpt, treePrescl[1]);
      if(is100) hpp_TrgObj100[1]->Fill(recpt, treePrescl[2]);      
...
int main(int argc, char *argv[]){
  ...
  if(argc!=8 && argc!=1){
    ...
    std::cout<<"./readForests_ppData_no40.exe";
    std::cout<<std::endl<<std::endl;
  ...
  // good input, run
  rStatus=1;
  if(argc==1) rStatus = readForests_ppData_no40();
    ...
    rStatus = readForests_ppData_no40( startfile, endfile, inputFileList, outputFileName,
