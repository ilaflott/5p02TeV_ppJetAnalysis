...
// compile with...
// g++ readForests_ppData_omitTrig.C $(root-config --cflags --libs) -Werror -Wall -O2 -o readForests_ppData_omitTrig.exe
...
const std::string defOutputName = "readForests_ppData_omitTrig_defOut.root";
const bool defDebugMode = true;
int readForests_ppData_omitTrig(int startfile = defStartFile , int endfile = defEndFile ,
...
// misc 
const bool omitJet40=false, omitJet60=false, omitJet80=false, omitJet100=true;
...
float trigComb(bool *trgDec, int *treePrescl, double triggerPt){
  float weight_eS=0.;
  if(!omitJet40 &&  (trgDec[0] && triggerPt>=40.  && triggerPt<60. )) weight_eS = treePrescl[0];
  if(!omitJet60 &&  (trgDec[1] && triggerPt>=60.  && triggerPt<80. )) weight_eS = treePrescl[1];
  if(!omitJet80 &&  (trgDec[2] && triggerPt>=80.  && triggerPt<100.)) weight_eS = treePrescl[2];
  if(!omitJet100 && (trgDec[3] && triggerPt>=100. )                 ) weight_eS = treePrescl[3];
  return weight_eS;
}
