...
int readForests_ppMC_MCJEC(std::string inFilelist , int startfile , int endfile ,  
  ...
  TH1F *hJER_rapbins_genptBin1[2][nbins_abseta]; int genptBin1Low=12, genptBin1High=15;
  TH1F *hJER_rapbins_genptBin2[2][nbins_abseta]; int genptBin2Low=84, genptBin2High=97;
  ...
  if(fillMCJECHists){        
    ...
    for(int bin = 0; bin<nbins_abseta; ++bin) {      
      ...
      hJER_rapbins[jtID][bin] = new TH1F(hTitleJER.c_str(),"matched, recpt/genpt, abs(y) bins", 100,0.,2.); 
      
      std::string hTitleJER_1="hJER_"+std::to_string(jtID)+"wJetID_jty_etabin"+std::to_string(bin)+
	"_genptBin1";
      hJER_rapbins_genptBin1[jtID][bin] = new TH1F(hTitleJER_1.c_str(),"matched, recpt/genpt, abs(y) bins", 100,0.,2.); 
      
      std::string hTitleJER_2="hJER_"+std::to_string(jtID)+"wJetID_jty_etabin"+std::to_string(bin)+
	"_genptBin2";
      hJER_rapbins_genptBin2[jtID][bin] = new TH1F(hTitleJER_2.c_str(),"matched, recpt/genpt, abs(y) bins", 100,0.,2.); 
    }
