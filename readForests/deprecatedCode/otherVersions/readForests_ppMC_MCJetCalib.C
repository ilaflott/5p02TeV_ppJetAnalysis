...
int readForests_ppMC(std::string inFilelist , int startfile , int endfile ,  
  ...
  for(UInt_t nEvt = 0; nEvt < NEvents_read; ++nEvt) {//event loop   
	...
	int jtID = 0; bool fillHists=false;	
	if(fillMCJetIDHists)  {
	  jtID=1;
	  if(passesJetID)fillHists=true;
	  else fillHists=false;	   } 
	else fillHists=true;
	...
	if(fillHists){
	  ...
	  if(etabin== nbins_eta-1) etabin=-1;
	  
	  if(etabin != -1) {
	    hJEC[jtID]->Fill( rawpt,eta_F[jet],(float)(recpt/rawpt), weight_eS );
	    if(genpt >= 30 && genpt<50) hJER_eta_30pt50[jtID][etabin]->Fill((float)recpt/genpt, weight_eS);
	    if(genpt >= 150 && genpt<200) hJER_eta_150pt200[jtID][etabin]->Fill((float)recpt/genpt, weight_eS);
	  ...
	  /////   GEN/RECO MATCHING   /////
	  //if(fillMCEffHists && !fillMCJetIDHists){
	  ...
	  //hpp_mceff_drjt[jtID]->Fill(gendrjt, recdrjt/gendrjt, weight_eS);
