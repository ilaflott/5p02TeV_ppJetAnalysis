// custom header
#include "readForests_ppData_omitTrig.h"
...
//// readForests_ppData_omitTrig
// ---------------------------------------------------------------------------------------------------------------
int readForests_ppData_omitTrig(int startfile , int endfile , std::string inFilelist , std::string outfile ,
  ...
  if(omitJet40)std::cout<<"omitting HLTjet40"<<std::endl;
  if(omitJet60)std::cout<<"omitting HLTjet60"<<std::endl;
  if(omitJet80)std::cout<<"omitting HLTjet80"<<std::endl;
  if(omitJet100)std::cout<<"omitting HLTjet100"<<std::endl;
  ...
  for(UInt_t nEvt = 0; nEvt < NEvents_read; ++nEvt) {//event loop   
    ...
    if(!omitJet40  && jet40_I){ for(unsigned int itt=0; itt<trgObj40_size; ++itt){
	if(trgObjpt_40->at(itt) > triggerPt) 
	  triggerPt = trgObjpt_40->at(itt);       } }
    if(!omitJet60  && jet60_I){ for(unsigned int itt=0; itt<trgObj60_size; ++itt){
	if(trgObjpt_60->at(itt) > triggerPt) 
	  triggerPt = trgObjpt_60->at(itt);       } }
    if(!omitJet80  && jet80_I){ for(unsigned int itt=0; itt<trgObj80_size; ++itt){
    	if(trgObjpt_80->at(itt) > triggerPt) 
	  triggerPt = trgObjpt_80->at(itt);       } }
    if(!omitJet100 && jet100_I){ for(unsigned int itt=0; itt<trgObj100_size; ++itt){
    ...
    if( !omitJet40  && (trgDec[0] && triggerPt>=40.  && triggerPt<60.  )) is40  = true;    
    if( !omitJet60  && (trgDec[1] && triggerPt>=60.  && triggerPt<80.  )) is60  = true;    
    if( !omitJet80  && (trgDec[2] && triggerPt>=80.  && triggerPt<100. )) is80  = true;    
    if( !omitJet100 && (trgDec[3] && triggerPt>=100.                   )) is100 = true;    
...
int main(int argc, char *argv[]){
  ...
  if(argc!=8 && argc!=1){
    ...
    std::cout<<"./readForests_ppData_omitTrig.exe";
    std::cout<<std::endl<<std::endl;
  ...
  // good input, run
  rStatus=1;
  if(argc==1) rStatus = readForests_ppData_omitTrig();
    ...
    rStatus = readForests_ppData_omitTrig( startfile, endfile, inputFileList, outputFileName,
