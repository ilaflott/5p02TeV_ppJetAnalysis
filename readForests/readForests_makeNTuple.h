...
const std::string defDataOutputName="readForests_ppData_makeNTuple_defOut.root";
const std::string defMCOutputName="readForests_ppMC_defOut";//.root";
const float defEtaCutLo=0.0, defEtaCutHi=4.7;

int readForests_ppData_makeNTuple( std::string inFilelist=defDataInFilelist, 
...
//int readForests_ppMC_makeNTuple( std::string inFilelist=defMCInFilelist,
//			       int startfile=0, int endfile=4, 
//			       int radius=defRadius, std::string jetType=defJetType, 
//			       bool debugMode=defDebugMode,			
//			       std::string outfile=(defMCOutputName+"_makeNTuple.root"),
//			       float jtEtaCutLo=defEtaCutLo, float jtEtaCutHi=defEtaCutHi      );

const int readForestsArgCount=9+minArgs;

// extended eta range for jetID Eff, or more QA in diff region... etc.
const float jetQAPtCut=40.;
const float jtPtCut_Hi=9999.; 
