...
int readForests_ppData( std::string inFilelist , int startfile , int endfile , 
  ...
  if(debugMode)for(int i=0;i<N_HLTBits;i++){std::cout<<std::endl;
  ...
  // i dont think this will work, i will try it anyways.
  //prescale/decision arrays, total prescale=L1_ps*HLT_ps
  int PFtrgDec[N_HLTBits]   ={ PFJet40_I, PFJet60_I, PFJet80_I, PFJet100_I };
  ...
  int CalotrgDec[N_HLTBits]   ={ CaloJet40_I, CaloJet60_I, CaloJet80_I, CaloJet100_I};
  ...
  if(trgCombType=="Calo"){
    for(int k=0;k<N_HLTBits;++k){ trgDec[k]=(bool)CalotrgDec[k]; trgPscl[k]=CalotrgPrescl[k]; } }
  else if (trgCombType=="PF"){
    for(int k=0;k<N_HLTBits;++k){ trgDec[k]=(bool)PFtrgDec[k]; trgPscl[k]=PFtrgPrescl[k]; } }
  else assert(false);// unsure of trigger combination being attempted, abort.
  ...
  for(UInt_t nEvt = 0; nEvt < NEvents_read; ++nEvt) {
    ...
    if(trgDec[3])
      for(unsigned int itt=0; itt<trgObj100_size; ++itt)
    	if(trgObjpt_100->at(itt) > trgPt) trgPt = trgObjpt_100->at(itt);
    if(trgDec[2])
      for(unsigned int itt=0; itt<trgObj80_size; ++itt)
    	if(trgObjpt_80->at(itt) > trgPt) trgPt = trgObjpt_80->at(itt); 
    if(trgDec[1]) 
      for(unsigned int itt=0; itt<trgObj60_size; ++itt)
	if(trgObjpt_60->at(itt) > trgPt) trgPt = trgObjpt_60->at(itt);       
    if(trgDec[0])  
      for(unsigned int itt=0; itt<trgObj40_size; ++itt)	
	if(trgObjpt_40->at(itt) > trgPt) trgPt = trgObjpt_40->at(itt); 
    ...
    if( trgDec[3] && trgPt>=100.               ) is100 = true;    
    if( trgDec[2] && trgPt>=80.  && trgPt<100. ) is80  = true;    
    if( trgDec[1] && trgPt>=60.  && trgPt<80.  ) is60  = true;    
    if( trgDec[0] && trgPt>=40.  && trgPt<60.  ) is40  = true;    
    ...
    float weight_eS = trigComb(trgDec, trgPscl, trgPt); //kurt method
    //float weight_eS = trigComb_sanityCheck(trgDec, treePrescl, trgPt); //kurt method
