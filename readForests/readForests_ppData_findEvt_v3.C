...
//// readForests_ppData_findEvt_v3
// ---------------------------------------------------------------------------------------------------------------
int readForests_ppData_findEvt_v3( std::string inFilelist , int startfile , int endfile , 
  ...
  for(UInt_t nEvt = 0; nEvt < NEvents_toRead; ++nEvt) {
    ...
    if(run_I!=262163)continue;
      ...
      if(lumi_I >20 || lumi_I<1)continue;
      else{
	//bool evtIsInList=findEvtInJohnsList(lumi_I,evt_I);
	bool evtIsInList=true;
    ...
    if(trgDec[2]){
      std::cout<<"Fired Trigger: HLT_AK4PFJet80_Eta5p1_v1 Prescale="<<trgPscl[2]<<std::endl;
    ...
    if(trgDec[1]) {
      std::cout<<"Fired Trigger: HLT_AK4PFJet60_Eta5p1_v1 Prescale="<<trgPscl[1]<<std::endl;
      std::cout<<"HLT objects:"<<std::endl;
    ...
    if(trgDec[0])  {
      std::cout<<"Fired Trigger: HLT_AK4PFJet40_Eta5p1_v1 Prescale="<<trgPscl[0]<<std::endl;
      std::cout<<"HLT objects:"<<std::endl;
    ...
    if((!firedTrigger) && false)continue;
    ...
    std::cout<<"Number of ak4PFJets in tree branch="<<nref_I<<std::endl;
	...
	if(fillDataJetJECUncHists ){
	  ...
	  for(int uu=0; uu<1; uu++){//loop so that if the jet doesn't meet some criteria i can just quit while i'm ahead
	    ...
	    if(fillDataJetJECUncHists&&fillDataJetCovMatrix){
  ...
  std::cout<<"readForests_ppData_findEvt_v3 finished."<<std::endl;  timer.Stop();
  ...
  return 0;
}    // end readForests_ppData_findEvt_v3
...
int main(int argc, char *argv[]){
  ...
  if(argc!=8 && argc!=1 && argc!=10){
    ...
    std::cout<<"./readForests_ppData_findEvt_v3.exe ";
  ...
  // good input, run
  rStatus=1;
  if(argc==1) rStatus = readForests_ppData_findEvt_v3();
    ...
    rStatus = readForests_ppData_findEvt_v3( inputFileList, startfile, endfile, 
