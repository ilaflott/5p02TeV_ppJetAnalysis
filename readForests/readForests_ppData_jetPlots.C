...
const bool fillDataRunLumiQAHists=false&&fillDataEvtQAHists;
...
int readForests_ppData_jetPlots( std::string inFilelist , int startfile , int endfile , 
  ...
  TH1D *h_NEvents_read     = new TH1D("NEvents_read"     , "NEvents read",    2,0.,2.); // # events looped over
  TH1D *h_NEvents_skimCut  = new TH1D("NEvents_skimCut"  , "NEvents read post skimCut",  2,0.,2.); // # events passing skim cut
  TH1D *h_NEvents_vzCut    = new TH1D("NEvents_vzCut"    , "NEvents read post vzCut AND skimCut", 2,0.,2.); // # events passing vz cut & skim cut
  ...
  TH1D *h_NEvents_trigd_incl_L1J28 = new TH1D("NEvents_trigs_incl_L1J28", "NEvents post quality, firing L1J28 bit",       2,0.,2.);//fills only if L1 bit fires        
  TH1D *h_NEvents_trigd_incl_HLT40 = new TH1D("NEvents_trigs_incl_HLT40", "NEvents post quality, firing HLT40 bit",       2,0.,2.);//fills only if HLT bit fires       

  TH1D *h_NEvents_trigd_incl_L1J40 = new TH1D("NEvents_trigs_incl_L1J40", "NEvents post quality, firing L1J40 bit",       2,0.,2.);//fills only if L1 bit fires        
  TH1D *h_NEvents_trigd_incl_HLT60 = new TH1D("NEvents_trigs_incl_HLT60", "NEvents post quality, firing HLT60 bit",       2,0.,2.);//fills only if HLT bit fires       

  TH1D *h_NEvents_trigd_incl_L1J48 = new TH1D("NEvents_trigs_incl_L1J48", "NEvents post quality, firing L1J48 bit",       2,0.,2.);//fills only if L1 bit fires        
  TH1D *h_NEvents_trigd_incl_HLT80 = new TH1D("NEvents_trigs_incl_HLT80", "NEvents post quality, firing HLT80 bit",       2,0.,2.);//fills only if HLT bit fires       

  TH1D *h_NEvents_trigd_incl_HLT40or60 = new TH1D("NEvents_trigs_incl_HLT40or60", "NEvents post quality, firing either HLT40 or HLT60 incl", 2,0.,2.);//in case of HighPtJet80, this should be equivalent to the incl_trg80 hist when HLT100 not being used	  
                                                                                                                                          //in case of HighPtLowerJets, this will be basically incl_trg60_or_trg40 (not named this way)          

  TH1D *h_NEvents_trigd_excl_HLT40 = new TH1D("NEvents_trigs_excl_HLT40", "NEvents post quality, firing HLT40 exclusively", 2,0.,2.);//fills only if final trigger decision fires and trigger object pt criteria for hlt40 met
  TH1D *h_NEvents_trigd_excl_HLT60 = new TH1D("NEvents_trigs_excl_HLT60", "NEvents post quality, firing HLT60 exclusively", 2,0.,2.);//fills only if final trigger decision fires and trigger object pt criteria for hlt60 met
  TH1D *h_NEvents_trigd_excl_HLT80 = new TH1D("NEvents_trigs_excl_HLT80", "NEvents post quality, firing HLT80 exclusively", 2,0.,2.);//fills only if final trigger decision fires and trigger object pt criteria for hlt80 met
  TH1D *h_NEvents_trigd_excl_HLT40or60 = new TH1D("NEvents_trigs_excl_HLT40or60", "NEvents post quality, firing either HLT40 or HLT60 excl", 2,0.,2.);      //fills only if final trigger decision fires and trigger object pt criteria for an hlt decision of interest is met
                                                                                                                                                //in case of HighPtJet80, this should be equivalent to the excl_trg80 hist when HLT100 not being used	  
																		//in case of HighPtLowerJets, this will be basically excl_trg60_or_trg40 (not named this way)          
  ...
  TH1D *hJetQA[nbins_abseta][N_vars][2]={};    //for info *in forests* (except the jet mass)
  TH1D *hJetQA_jtpt_JEC_sysup[nbins_abseta]={};    
  TH1D *hJetQA_jtpt_JEC_sysdown[nbins_abseta]={};    

  //these hists *derived* from info in forests (except jet mass)
  TH1D *hJetQA_jtptybins[nbins_absy][2]={};
  TH1D *hJetQA_jtptybins_JEC_sysup[nbins_absy]={};
  TH1D *hJetQA_jtptybins_JEC_sysdown[nbins_absy]={};
  TH1D *hJetQA_jttheta[nbins_absy][2]={};//angle from beamline, 0 -- >pi
  TH1D *hJetQA_jtpz[nbins_absy][2]={};//z-component momentum
  TH1D *hJetQA_jtp[nbins_absy][2]={};//momentum magnitude
  TH1D *hJetQA_jtE[nbins_absy][2]={};//energy
  TH1D *hJetQA_jtEt[nbins_absy][2]={};//transverse energy
  TH1D *hJetQA_jtm[nbins_absy][2]={};//mass
  TH1D *hJetQA_jtmt[nbins_absy][2]={};//transverse mass
  TH1D *hJetQA_jty[nbins_absy][2]={};//rapidity (NOT pseudorapidity)
  TH2D* hJetQA_jteta_jty=NULL;
  if(fillDataJetQAHists){        
    //these hists *derived* from info in forests (except jet mass), for positive jet id only
    hJetQA_jteta_jty = new TH2D( "hJetQA_1wJetID_jteta_jty" , ";#eta^{Jet};y^{Jet}" , 55  ,0. ,5.5,
				 55  ,0. ,5.5 );       
    for(int i=0; i<2; i++){
      for(int k = 0; k<nbins_absy; ++k){      
	hJetQA_jtptybins[k][i]     = new TH1D( Form("hJetQA_%dwJetID_jtpt_ybin%d" , i , k) , ";p_{T}^{Jet} [GeV];" , 1500 ,0.     ,1500.);       
	hJetQA_jttheta[k][i]       = new TH1D( Form("hJetQA_%dwJetID_jttheta_ybin%d"   , i , k) , ";#theta^{Jet} [rad];"      , 200  ,0.     ,TMath::Pi());       
	hJetQA_jtpz[k][i]          = new TH1D( Form("hJetQA_%dwJetID_jtpz_ybin%d"      , i , k) , ";p_{z}^{Jet} [GeV];"       , 3000 ,-1500. ,1500.);      
	hJetQA_jtp[k][i]           = new TH1D( Form("hJetQA_%dwJetID_jtp_ybin%d"       , i , k) , ";#||{#bf{p^{Jet}}} [GeV];" , 1500 ,0.     ,1500.);       
	hJetQA_jtE[k][i]           = new TH1D( Form("hJetQA_%dwJetID_jtE_ybin%d"       , i , k) , ";E^{Jet} [GeV];"           , 1500 ,0.     ,1500.);       
	hJetQA_jtEt[k][i]          = new TH1D( Form("hJetQA_%dwJetID_jtEt_ybin%d"      , i , k) , ";E_{T}^{Jet} [GeV];"       , 1500 ,0.     ,1500.);       
	hJetQA_jtm[k][i]           = new TH1D( Form("hJetQA_%dwJetID_jtm_ybin%d"       , i , k) , ";m^{Jet} [GeV];"           , 1500 ,0.     ,1500.);       
	hJetQA_jtmt[k][i]          = new TH1D( Form("hJetQA_%dwJetID_jtmt_ybin%d"      , i , k) , ";m_{T}^{Jet};"             , 1500 ,0.     ,1500.);       
	hJetQA_jty[k][i]           = new TH1D( Form("hJetQA_%dwJetID_jty_ybin%d"       , i , k) , ";y^{Jet};"                 , 102  ,-5.1   ,5.1);       
	if(i==1 && fillDataJetJECUncHists){
	  hJetQA_jtptybins_JEC_sysup[k]   = new TH1D( Form("hJetQA_%dwJetID_jtpt_JEC_sysup_ybin%d"  , i,k) , "JEC sysup Jet p_{T};p_{T}^{Jet} [GeV];"  , 1500,0,1500);       
	  hJetQA_jtptybins_JEC_sysdown[k] = new TH1D( Form("hJetQA_%dwJetID_jtpt_JEC_sysdown_ybin%d", i,k) , "JEC sysdown Jet p_{T};p_{T}^{Jet} [GeV];", 1500,0,1500);       
	}//end JECUnc specifit
      }//end nbins absy loop
    
      //for info *in forests* (except the jet mass)
      for(int j = 0; j<N_vars; ++j){		      
	for(int k = 0; k<nbins_abseta; ++k){      
	  
	  //jets
	  if(var[j]=="jtpt"||var[j]=="rawpt"){
	    hJetQA[k][j][i] = new TH1D( Form("hJetQA_%dwJetID_%s_etabin%d", i,var[j].c_str(),k) , Form(";%s;", var[j].c_str()) , 1500,0,1500);       
	    if(i==1 && fillDataJetJECUncHists && var[j]=="jtpt"){
	      hJetQA_jtpt_JEC_sysup[k]   = new TH1D( Form("hJetQA_%dwJetID_%s_JEC_sysup_etabin%d"  , i,var[j].c_str(),k) , Form("JEC sysup Jet p_{T};%s;"  , var[j].c_str()) , 1500,0,1500);       
	      hJetQA_jtpt_JEC_sysdown[k] = new TH1D( Form("hJetQA_%dwJetID_%s_JEC_sysdown_etabin%d", i,var[j].c_str(),k) , Form("JEC sysdown Jet p_{T};%s;", var[j].c_str()) , 1500,0,1500);       
	    }
	  }
	  else if(var[j]=="jteta") 
  ...
  TH2D  *hpp_IncHLT40trgEtaPhi=NULL,*hpp_IncHLT40trgEtaPhi_allobj=NULL,*hpp_IncHLT40trgEtaPhi_leadobj=NULL;
  TH2D  *hpp_IncHLT60trgEtaPhi=NULL,*hpp_IncHLT60trgEtaPhi_allobj=NULL,*hpp_IncHLT60trgEtaPhi_leadobj=NULL;
  TH2D  *hpp_IncHLT80trgEtaPhi=NULL,*hpp_IncHLT80trgEtaPhi_allobj=NULL,*hpp_IncHLT80trgEtaPhi_leadobj=NULL;
  ...
  if(fillDataJetTrigQAHists){
    ...
    hpp_IncHLT40trgEtaPhi  = new TH2D( "IncHLT40trgEtaPhi"  , "HLTak4PFJet40;Trg. Obj. #eta;Trg. Obj. #phi", 		     Netasegments, (Double_t*)etasegments,		     72, -3.14159, 3.14159 );
    hpp_IncHLT60trgEtaPhi  = new TH2D( "IncHLT60trgEtaPhi"  , "HLTak4PFJet60;Trg. Obj. #eta;Trg. Obj. #phi", 		     Netasegments, (Double_t*)etasegments,		     72, -3.14159, 3.14159 );
    hpp_IncHLT80trgEtaPhi  = new TH2D( "IncHLT80trgEtaPhi"  , "HLTak4PFJet80;Trg. Obj. #eta;Trg. Obj. #phi", 		     Netasegments, (Double_t*)etasegments,		     72, -3.14159, 3.14159 );

    hpp_IncHLT40trgEtaPhi_leadobj  = new TH2D( "IncHLT40trgEtaPhi_leadobj"  , "HLTak4PFJet40;Lead Trg. Obj. #eta;Lead Trg. Obj. #phi", 		     Netasegments, (Double_t*)etasegments,		     72, -3.14159, 3.14159 );
    hpp_IncHLT60trgEtaPhi_leadobj  = new TH2D( "IncHLT60trgEtaPhi_leadobj"  , "HLTak4PFJet60;Lead Trg. Obj. #eta;Lead Trg. Obj. #phi", 		     Netasegments, (Double_t*)etasegments,		     72, -3.14159, 3.14159 );
    hpp_IncHLT80trgEtaPhi_leadobj  = new TH2D( "IncHLT80trgEtaPhi_leadobj"  , "HLTak4PFJet80;Lead Trg. Obj. #eta;Lead Trg. Obj. #phi", 		     Netasegments, (Double_t*)etasegments,		     72, -3.14159, 3.14159 );

    hpp_IncHLT40trgEtaPhi_allobj  = new TH2D( "IncHLT40trgEtaPhi_allobj"  , "HLTak4PFJet40 All Objects;Trg. Obj. #eta;Trg. Obj. #phi", 		     Netasegments, (Double_t*)etasegments,		     72, -3.14159, 3.14159 );
    hpp_IncHLT60trgEtaPhi_allobj  = new TH2D( "IncHLT60trgEtaPhi_allobj"  , "HLTak4PFJet60 All Objects;Trg. Obj. #eta;Trg. Obj. #phi", 		     Netasegments, (Double_t*)etasegments,		     72, -3.14159, 3.14159 );
    hpp_IncHLT80trgEtaPhi_allobj  = new TH2D( "IncHLT80trgEtaPhi_allobj"  , "HLTak4PFJet80 All Objects;Trg. Obj. #eta;Trg. Obj. #phi", 		     Netasegments, (Double_t*)etasegments,		     72, -3.14159, 3.14159 );
  ...
  TH2D* hpp_covmat_y_arr[nbins_absy]={};
  TH1D* hpp_covmat_y_arr_helpers[nbins_absy]={};
  bool ybin_bool_helper[nbins_absy]={0};
  
  TH2D* hpp_covmat_y_arr_JEC_sysup[nbins_absy]={};
  TH1D* hpp_covmat_y_arr_helpers_JEC_sysup[nbins_absy]={};
  TH2D* hpp_covmat_y_arr_JEC_sysdown[nbins_absy]={};
  TH1D* hpp_covmat_y_arr_helpers_JEC_sysdown[nbins_absy]={};
  bool JEC_ybin_bool_helper[nbins_absy]={0};
  
  if(fillDataJetCovMatrix){
    for(int i=0;i<nbins_abseta;i++){
      int     nbins_pt_covmat=-1;
      double* boundaries_pt_covmat=setBinning_etabin(i,  &nbins_pt_covmat);      

      std::string covmat_eta_title="Data Jet Covariance Matrix, "+absetabins_str[i]+" < #||{#eta} < "+absetabins_str[i+1];    
      ...
      hpp_covmat_eta_arr[i]=new TH2D(covmat_eta_name.c_str(), covmat_eta_title.c_str(), nbins_pt_covmat, boundaries_pt_covmat, nbins_pt_covmat, boundaries_pt_covmat);     
      hpp_covmat_eta_arr_helpers[i]=new TH1D(("covmat_helper_etabin"+std::to_string(i)).c_str(), "helper only", nbins_pt_covmat, boundaries_pt_covmat);      
	...
	hpp_covmat_eta_arr_JEC_sysup[i]=new TH2D((covmat_eta_name_JEC_sysup).c_str(), ("JEC sysup "+covmat_eta_title).c_str(), nbins_pt_covmat, boundaries_pt_covmat, nbins_pt_covmat, boundaries_pt_covmat);     
	hpp_covmat_eta_arr_helpers_JEC_sysup[i]=new TH1D(("covmat_helper_JEC_sysup_etabin"+std::to_string(i)).c_str(), "helper only", nbins_pt_covmat, boundaries_pt_covmat);      
	...
	hpp_covmat_eta_arr_JEC_sysdown[i]=new TH2D((covmat_eta_name_JEC_sysdown).c_str(), ("JEC sysdown "+covmat_eta_title).c_str(), nbins_pt_covmat, boundaries_pt_covmat, nbins_pt_covmat, boundaries_pt_covmat);     
	hpp_covmat_eta_arr_helpers_JEC_sysdown[i]=new TH1D(("covmat_helper_JEC_sysdown_etabin"+std::to_string(i)).c_str(), "helper only", nbins_pt_covmat, boundaries_pt_covmat);      
      }
    }//nbinsabseta
    
    for(int i=0;i<nbins_absy;i++){
      int     nbins_pt_covmat=-1;
      double* boundaries_pt_covmat=setBinning_etabin(i,  &nbins_pt_covmat);

      std::string covmat_y_title="Data Jet Covariance Matrix, "+absybins_str[i]+" < #||{y} < "+absybins_str[i+1];    
      if(debugMode)std::cout<<"covmat_y_title =" << covmat_y_title<< std::endl;      
      std::string covmat_y_name="hpp_covmat_";
      //if(fillDataJetIDHists)covmat_y_name+="wJetID_";
      covmat_y_name+="wJetID_";
      covmat_y_name+="ybin_"+std::to_string(i);      
      if(debugMode)std::cout<<"covmat_y_name  =" << covmat_y_name << std::endl;      
      hpp_covmat_y_arr[i]=new TH2D(covmat_y_name.c_str(), covmat_y_title.c_str(), nbins_pt_covmat, boundaries_pt_covmat, nbins_pt_covmat, boundaries_pt_covmat);     
      hpp_covmat_y_arr_helpers[i]=new TH1D(("covmat_helper_ybin"+std::to_string(i)).c_str(), "helper only", nbins_pt_covmat, boundaries_pt_covmat);      

      if(fillDataJetJECUncHists){
	std::string covmat_y_name_JEC_sysup="hpp_covmat_";
	//if(fillDataJetIDHists)covmat_y_name+="wJetID_";
	covmat_y_name_JEC_sysup+="wJetID_JEC_sysup_";
	covmat_y_name_JEC_sysup+="ybin_"+std::to_string(i);      
	if(debugMode)std::cout<<"covmat_y_name_JEC_sysup  =" << covmat_y_name_JEC_sysup << std::endl;      
	hpp_covmat_y_arr_JEC_sysup[i]=new TH2D((covmat_y_name_JEC_sysup).c_str(), ("JEC sysup "+covmat_y_title).c_str(), nbins_pt_covmat, boundaries_pt_covmat, nbins_pt_covmat, boundaries_pt_covmat);     
	hpp_covmat_y_arr_helpers_JEC_sysup[i]=new TH1D(("covmat_helper_JEC_sysup_ybin"+std::to_string(i)).c_str(), "helper only", nbins_pt_covmat, boundaries_pt_covmat);      

	std::string covmat_y_name_JEC_sysdown="hpp_covmat_";
	//if(fillDataJetIDHists)covmat_y_name+="wJetID_";
	covmat_y_name_JEC_sysdown+="wJetID_JEC_sysdown_";
	covmat_y_name_JEC_sysdown+="ybin_"+std::to_string(i);      
	if(debugMode)std::cout<<"covmat_y_name_JEC_sysdown  =" << covmat_y_name_JEC_sysdown << std::endl;      
	hpp_covmat_y_arr_JEC_sysdown[i]=new TH2D((covmat_y_name_JEC_sysdown).c_str(), ("JEC sysdown "+covmat_y_title).c_str(), nbins_pt_covmat, boundaries_pt_covmat, nbins_pt_covmat, boundaries_pt_covmat);     
	hpp_covmat_y_arr_helpers_JEC_sysdown[i]=new TH1D(("covmat_helper_JEC_sysdown_ybin"+std::to_string(i)).c_str(), "helper only", nbins_pt_covmat, boundaries_pt_covmat);      
      }//JEC Unc specific	
    }//nbins absy
  ...
  //END particle-flow tree relevant histograms only here
  
  // EVENT LOOP + PREP
  // declare variables/arrays + set branch address for each input tree
  //JetAnalyzer, jets, in tree
  int nref_I;  
...
//  // skimanalysis
//  int pBeamScrapingFilter_I, pHBHENoiseFilter_I, pprimaryvertexFilter_I,puvertexFilter_I;//, pHBHEIsoNoiseFilter_I;
//  //jetpp[2]->SetBranchAddress("HBHENoiseFilterResult",&pHBHENoiseFilter_I);
//  //jetpp[2]->SetBranchAddress("HBHENoiseFilterResultRun1",&pHBHENoiseFilter_I);
//  jetpp[2]->SetBranchAddress("HBHENoiseFilterResultRun2Loose",&pHBHENoiseFilter_I);//what i have been using
//  //jetpp[2]->SetBranchAddress("HBHENoiseFilterResultRun2Tight",&pHBHENoiseFilter_I);
//  
//  //jetpp[2]->SetBranchAddress("HBHEIsoNoiseFilterResult",&pHBHEIsoNoiseFilter_I);//only option
//  jetpp[2]->SetBranchAddress("pPAprimaryVertexFilter",&pprimaryvertexFilter_I);//only option, what i have been using
//  jetpp[2]->SetBranchAddress("pBeamScrapingFilter",&pBeamScrapingFilter_I);//only option, what i have been using
...
//  //jetpp[2]->SetBranchAddress("pVertexFilterCutEandG",&puvertexFilter_I);
  ...
  jetpp[4]->SetBranchAddress( L1Branches[0].c_str()  , &jet40_l1s_I);   //l1jet28 for HLT40
  jetpp[4]->SetBranchAddress( L1Branches[1].c_str()  , &jet60_l1s_I);   //l1jet40 for HLT60
  jetpp[4]->SetBranchAddress( L1Branches[2].c_str()  , &jet80_l1s_I);   //l1jet48 for HLT80
  jetpp[4]->SetBranchAddress( L1Branches[3].c_str()  , &jet100_l1s_I);  //l1jet52 for HLT100
  ...
  if(useTupel){
    ...
    jetpp[6]->SetBranchAddress( "JetAk04Pt",                  &jetPt                  );
    jetpp[6]->SetBranchAddress( "JetAk04Eta",                 &jetEta                 );
    jetpp[6]->SetBranchAddress( "JetAk04Phi",                 &jetPhi                 );
    jetpp[6]->SetBranchAddress( "JetAk04E",                   &jetE                   );
    jetpp[6]->SetBranchAddress( "JetAk04Id",                  &jetID                  );
    jetpp[6]->SetBranchAddress( "JetAk04RawPt",               &jetRawPt               );
    jetpp[6]->SetBranchAddress( "JetAk04RawE",                &jetRawE                );
    jetpp[6]->SetBranchAddress( "JetAk04HfHadE",              &jetHfHadE              );
    jetpp[6]->SetBranchAddress( "JetAk04HfEmE",               &jetHfEmE               );
    
    jetpp[6]->SetBranchAddress( "JetAk04ChHadFrac",           &jetChHadFrac           );
    jetpp[6]->SetBranchAddress( "JetAk04NeutralHadAndHfFrac", &jetNeutralHadAndHfFrac );
    jetpp[6]->SetBranchAddress( "JetAk04ChEmFrac",            &jetChEmFrac            );
    jetpp[6]->SetBranchAddress( "JetAk04NeutralEmFrac",       &jetNeutralEmFrac       );
    jetpp[6]->SetBranchAddress( "JetAk04ChMult",              &jetChMult              );
    jetpp[6]->SetBranchAddress( "JetAk04ConstCnt",            &jetConstCnt            );
  }
...
//    if(nEvt>152)assert(false);
    ...
    double HLT40maxTrgPhi=-1.,  HLT60maxTrgPhi=-1.,  HLT80maxTrgPhi=-1.;//, HLT100maxTrgPhi=-1.; 
	...
	hpp_IncHLT80trgEtaPhi_allobj->Fill(trgeta,trgphi);
      }
	...
	double trgeta=trgObjeta_80->at(itt);
	double trgphi=trgObjphi_80->at(itt);	
	hpp_IncHLT80trgEtaPhi->Fill(trgeta,trgphi);
	if(trgpt > HLT80maxTrgPt){
	  HLT80maxTrgPt=trgpt;
	  HLT80maxTrgEta=trgeta;	
	  HLT80maxTrgPhi=trgphi;	
	}
      }
    }
	...
	hpp_IncHLT60trgEtaPhi_allobj->Fill(trgeta,trgphi);
      }
	...
	double trgeta=trgObjeta_60->at(itt);
	double trgphi=trgObjphi_60->at(itt);	
	hpp_IncHLT60trgEtaPhi->Fill(trgeta,trgphi);
	if(trgpt > HLT60maxTrgPt){	  
	  HLT60maxTrgPt=trgpt;
	  HLT60maxTrgEta=trgeta;
	  HLT60maxTrgPhi=trgphi;	
	}    
      }
    }
	...
	hpp_IncHLT40trgEtaPhi_allobj->Fill(trgeta,trgphi);
      }
	...
	double trgeta=trgObjeta_40->at(itt);
	double trgphi=trgObjphi_40->at(itt);	
	hpp_IncHLT40trgEtaPhi->Fill(trgeta,trgphi);
	if(trgpt > HLT40maxTrgPt){
	  HLT40maxTrgPt=trgpt;
	  HLT40maxTrgEta=trgeta;	
	  HLT40maxTrgPhi=trgphi;	
	}
      }
    }
    ...
    if(filelistIsMinBias){
      if(isMB)	weight_eS/=effMBLumi;
      else continue;    }
    else if(filelistIsJet80){
      if((bool)jet80_l1s_I)h_NEvents_trigd_incl_L1J48->Fill(0.);
      if((bool)Jet80_I)    h_NEvents_trigd_incl_HLT80->Fill(0.);
      if(is80){
	weight_eS/=effJet80Lumi;
	h_NEvents_trigd_excl_HLT80->Fill(0.);
      }
      else continue;    }      
    else if(filelistIsLowerJets){
      if((bool)jet60_l1s_I)h_NEvents_trigd_incl_L1J40->Fill(0.);
      if((bool)Jet60_I)    h_NEvents_trigd_incl_HLT60->Fill(0.);
      if((bool)jet40_l1s_I)h_NEvents_trigd_incl_L1J28->Fill(0.);
      if((bool)Jet40_I)    h_NEvents_trigd_incl_HLT40->Fill(0.);
      if(trgDec[0] || trgDec[1]) h_NEvents_trigd_incl_HLT40or60->Fill(0.);      
      if(is40||is60)	{	
	weight_eS/=effLowJetsLumi;
	if(is40)	h_NEvents_trigd_excl_HLT40->Fill(0.);
	else if(is60)	h_NEvents_trigd_excl_HLT60->Fill(0.);
	h_NEvents_trigd_excl_HLT40or60->Fill(0.);
      }
      else continue;    }
    else assert(false);
    ...
    if     ( isMB  )  {   h_NEvents_isMB ->Fill(0.)  ;  h_NEvents_isMB ->Fill(1.,weight_eS ) ; }//minbias
    else if( is40  )  {   h_NEvents_is40 ->Fill(0.)  ;  h_NEvents_is40 ->Fill(1.,weight_eS ) ; }                        
    else if( is60  )  {   h_NEvents_is60 ->Fill(0.)  ;  h_NEvents_is60 ->Fill(1.,weight_eS ) ; }
    else if( is80  )  {   h_NEvents_is80 ->Fill(0.)  ;  h_NEvents_is80 ->Fill(1.,weight_eS ) ; }
    else if( is100 )  {   h_NEvents_is100->Fill(0.)  ;  h_NEvents_is100->Fill(1.,weight_eS ) ; }
    ...
    if(fillDataJetTrigQAHists){ //only want to fill these trigger jet plots if they pass all our quality criteria      
      ...
      if(HLT40maxTrgPt>0.) hpp_IncHLT40trgEtaPhi_leadobj->Fill(HLT40maxTrgEta,HLT40maxTrgPhi);
      if(HLT60maxTrgPt>0.) hpp_IncHLT60trgEtaPhi_leadobj->Fill(HLT60maxTrgEta,HLT60maxTrgPhi);
      if(HLT80maxTrgPt>0.) hpp_IncHLT80trgEtaPhi_leadobj->Fill(HLT80maxTrgEta,HLT80maxTrgPhi);
	...
	float jetIDpt=rawpt;//ala HIN jetID, jtpt is corrected w/ L2/L3 residuals
	
	if (!(absreceta > 2.4))  //this is the usual jetID, loose based on 2015 13 TeV criteria 
...
//	if (!(absreceta > 2.4))  //this is the usual jetID, tight based on 2015 13 TeV criteia
//	  passesJetID=(bool)jetID_00eta24( jtpt, 
//					   neSum_F[jet],  phSum_F[jet],  chSum_F[jet],  eSum_F[jet], muSum_F[jet],
//					   numConst,  chMult, true);
//	if (!(absreceta > 2.4))  //this is the usual loose pseudo-2015-13 TeV jetID, w/ eMax/jtpt<0.89. It works well
//	  passesJetID=(bool)jetID_00eta24_TEST( jtpt, 
//						neSum_F[jet],  phSum_F[jet],  chSum_F[jet],  eSum_F[jet], muSum_F[jet],
//						numConst,  chMult, eMax_F[jet], useTightJetID);
//	if (!(absreceta > 2.4))  //this is the usual loose pseudo-2015-13 TeV jetID, w/ eSum/jtpt<0.89. It's largely the same as using the eMax criteria
//	  passesJetID=(bool)jetID_00eta24_TEST( jtpt, 
//						neSum_F[jet],  phSum_F[jet],  chSum_F[jet],  eSum_F[jet], muSum_F[jet],
//						numConst,  chMult, eSum_F[jet], useTightJetID);
	...
	//if(isinhotspot(receta,recphi))
	//  continue;	  
	
	
	// get pseudorapidity bin
	int theEtaBin=-1;	
	for(int etabin=0;etabin<nbins_abseta;++etabin)
	  if( absreceta>=absetabins[etabin]  && 		
	...
	if(fillDataJetJECQAHists && passesJetID ){
	  hjtpt[theEtaBin]->Fill(jtpt,weight_eS);//spectra
	  hL2L3Res_rawpt[theEtaBin]->Fill(L2L3Res_rawpt,weight_eS);
	  hL2L3Res_2_rawpt[theEtaBin]->Fill(L2L3Res_2_rawpt,weight_eS);
	  
	  hjtpt_L2L3Res_rawpt_diff[theEtaBin]->Fill((jtpt-L2L3Res_rawpt),weight_eS);//diff
	  hjtpt_L2L3Res_2_rawpt_diff[theEtaBin]->Fill((jtpt-L2L3Res_2_rawpt),weight_eS);
	  hL2L3Res_rawpt_L2L3Res_2_rawpt_diff[theEtaBin]->Fill((L2L3Res_rawpt-L2L3Res_2_rawpt),weight_eS);
	  
	  hjtpt_L2L3Res_rawpt_ratio[theEtaBin]->Fill((jtpt),weight_eS);//ratios (to be divided by corresponding spectra post processing)
	  hjtpt_L2L3Res_2_rawpt_ratio[theEtaBin]->Fill((jtpt),weight_eS);
	  hL2L3Res_rawpt_L2L3Res_2_rawpt_ratio[theEtaBin]->Fill((L2L3Res_rawpt),weight_eS);
	  
	  hjtpt_v_diff_L2L3Res_rawpt[theEtaBin]->Fill(jtpt, (jtpt-L2L3Res_rawpt), weight_eS);//pt v. diff TH2
	  hjtpt_v_diff_L2L3Res_2_rawpt[theEtaBin]->Fill(jtpt, (jtpt-L2L3Res_2_rawpt), weight_eS);
	  hL2L3Res_rawpt_v_diff_L2L3Res_2_rawpt[theEtaBin]->Fill(L2L3Res_rawpt, (L2L3Res_rawpt-L2L3Res_2_rawpt), weight_eS);
	}
	...
	if(fillDataJetJECUncHists && passesJetID){
	  ...
	  for(int uu=0; uu<1; uu++){//loop so that if the jet doesn't meet some criteria i can just quit while i'm ahead
	    ...
	    //assert(false);
	    hJetQA_jtpt_JEC_sysup[theEtaBin]  ->Fill(jtpt_sysup,  weight_eS);
	    hJetQA_jtpt_JEC_sysdown[theEtaBin]->Fill(jtpt_sysdown,weight_eS);
	    hJetQA_jtptybins_JEC_sysup[theYBin]  ->Fill(jtpt_sysup,  weight_eS);
	    hJetQA_jtptybins_JEC_sysdown[theYBin]->Fill(jtpt_sysdown,weight_eS);
	    if(fillDataJetCovMatrix){
	      JEC_etabin_bool_helper[theEtaBin]=true;      
	      hpp_covmat_eta_arr_helpers_JEC_sysup[theEtaBin]->Fill(jtpt_sysup);//,weight_eS);	    
	      hpp_covmat_eta_arr_helpers_JEC_sysdown[theEtaBin]->Fill(jtpt_sysdown);//,weight_eS);	    
	      JEC_ybin_bool_helper[theYBin]=true;      
	      hpp_covmat_y_arr_helpers_JEC_sysup[theYBin]->Fill(jtpt_sysup);//,weight_eS);	    
	      hpp_covmat_y_arr_helpers_JEC_sysdown[theYBin]->Fill(jtpt_sysdown);//,weight_eS);	    
	    }      
	    //assert(false);
	...
	if(fillDataRunLumiQAHists && passesJetID){
	...
	for(int jtid=0; jtid<2; jtid++){
	  ...
	  if(jtid==1 ){
	    ...
	    if(fillDataJetCovMatrix){
	      ...
	      hpp_covmat_eta_arr_helpers[theEtaBin]->Fill(jtpt);//,weight_eS);	    		      
	      ybin_bool_helper[theYBin]=true;      
	      hpp_covmat_y_arr_helpers[theYBin]->Fill(jtpt);//,weight_eS);	    		      
	    }
	  }//end jtid==1 specific. 	  
	  ...
	  if(fillDataJetTrigQAHists){
	    ...
	    hpp_HLTComb[theEtaBin][jtid]->Fill(jtpt, weight_eS); //kurts method
	    
	    //no exclusion between triggers
	    if(MBtrgDec ){
	      hpp_HLTMB[theEtaBin][jtid]->Fill(jtpt, (double)MBtrgPscl); //minbias
	      if( trgPt < HLTthresh[0] ) {
		hpp_incHLTMB[theEtaBin][jtid]->Fill(jtpt, (double)MBtrgPscl); //minbisa
	      }
	    }
	    
	    if(trgDec[0]){
	      hpp_HLT40[theEtaBin][jtid]->Fill(jtpt, (double)trgPscl[0]);  
		...
		hpp_incHLT40[theEtaBin][jtid]->Fill(jtpt, (double)trgPscl[0]);  
	      }		
	    }
	    
	    if(trgDec[1] ){
	      hpp_HLT60[theEtaBin][jtid]->Fill(jtpt, (double)trgPscl[1]);  
	      if( fabs(eta_F[jet])<0.5 && jtid==1 && passesJetID ){
		if(!MBtrgDec)hpp_incHLT60_NOT_HLTMB->Fill(jtpt, (double)trgPscl[1]);
	      }		
	      if( (trgPt>HLTthresh[1]) && (trgPt<HLTthresh[2]))  { 
		hpp_incHLT60[theEtaBin][jtid]->Fill(jtpt, (double)trgPscl[1]);  
	      }
	    }

	    if(trgDec[2]){
	      hpp_HLT80[theEtaBin][jtid]->Fill(jtpt, (double)trgPscl[2]);  
	      if( fabs(eta_F[jet])<0.5 && jtid==1 && passesJetID ){
		...
		hpp_incHLT80[theEtaBin][jtid]->Fill(jtpt, (double)trgPscl[2]);  
	      }
	    }
	    
	    if(trgDec[3]){
	      hpp_HLT100[theEtaBin][jtid]->Fill(jtpt, (double)trgPscl[3]); 
	      if(!(trgPt<HLTthresh[3]))  { //this would have to change if useHLT100 was turned on
		hpp_incHLT100[theEtaBin][jtid]->Fill(jtpt, (double)trgPscl[3]); 
	      } 
	    }
	    
	    //with exclusion
	    if(isMB)   {
	      hpp_excHLTMB[theEtaBin][jtid]->Fill( jtpt , weight_eS);	//minbias	
	      if( fabs(eta_F[jet])<0.5 && jtid==1 && passesJetID ){
		...
		if(!((bool)L1_HF_coincidence_I)) hpp_excHLTMB_NOT_HFcoincidence->Fill(jtpt,weight_eS);
	      ...
	      hpp_excHLT40[theEtaBin][jtid]->Fill( jtpt , weight_eS);
	      if( fabs(eta_F[jet])<0.5 && jtid==1 && passesJetID ){
		if(!MBtrgDec)hpp_excHLT40_NOT_HLTMB->Fill(jtpt, weight_eS);
	      }
	    }
	    else if(is60 ) { 
	      hpp_excHLT60[theEtaBin][jtid]->Fill( jtpt , weight_eS);
	      if( fabs(eta_F[jet])<0.5 && jtid==1 && passesJetID ){
		if(!MBtrgDec)hpp_excHLT60_NOT_HLTMB->Fill(jtpt, weight_eS);
	      }
	    }
	    else if(is80 ) { 
	      hpp_excHLT80[theEtaBin][jtid]->Fill( jtpt , weight_eS);
	      if( fabs(eta_F[jet])<0.5 && jtid==1 && passesJetID ){
		if(!MBtrgDec)hpp_excHLT80_NOT_HLTMB->Fill(jtpt, weight_eS);
	      }
	    }
	    else if(is100) { 
	      hpp_excHLT100[theEtaBin][jtid]->Fill( jtpt , weight_eS); 	    //assert(false);
	    }
	  ...
	  if(fillDataJetQAHists){
	    ...
	    int ind=0;
	    hJetQA_jtptybins[theYBin][jtid]->Fill(jtpt,weight_eS);
	    
	    ////TEMP DO NOT LEAVE ME IN THERE UNCOMMENTED//
	    //if(jtpt>500. && 
	    //   theYBin==2 && 
	    //   jtid== 1){
	    //  //run/evt/lumi = 262326/51/70062486
	    //  std::cout<<"I FOUND THE JET!!!!"<<std::endl;
	    //  std::cout<<"run/lumi/evt = "<< run_I<<"/"<<lumi_I<<"/"<<evt_I<<std::endl;
	    //  std::cout<<"jtpt="<<jtpt<<std::endl;
	    //  std::cout<<"jty="<<jty<<std::endl;
	    //  std::cout<<"recphi="<<recphi<<std::endl;
	    //  std::cout<<"receta="<<receta<<std::endl;
	    //  std::cout<<"is40="<<is40<<std::endl;
	    //  std::cout<<"trgPscl[0]="<<trgPscl[0]<<std::endl;	      
	    //  std::cout<<"# const.     = " <<numConst<<std::endl;	      
	    //  std::cout<<"# chg const. = " <<chMult<<std::endl;
	    //  std::cout<<"     chg had frac. ="<<chSum_F[jet]/jtpt<<std::endl;
	    //  std::cout<<" MAX chg had frac. ="<<chMax_F[jet]/jtpt<<std::endl;
	    //  std::cout<<"    electron frac. ="<<eSum_F[jet]/jtpt<<std::endl;
	    //  std::cout<<"MAX electron frac. ="<<eMax_F[jet]/jtpt<<std::endl;
	    //  std::cout<<"        muon frac. ="<<muSum_F[jet]/jtpt<<std::endl;	      
	    //  std::cout<<"    MAX muon frac. ="<<muMax_F[jet]/jtpt<<std::endl;	      
	    //  std::cout<<"# neu const. = " <<neuMult<<std::endl;	    	    	   	    
	    //  std::cout<<"     neu had frac. ="<<neSum_F[jet]/jtpt<<std::endl;
	    //  std::cout<<" MAX neu had frac. ="<<neMax_F[jet]/jtpt<<std::endl;
	    //  std::cout<<"      photon frac. ="<<phSum_F[jet]/jtpt<<std::endl;	      
	    //  std::cout<<"  MAX photon frac. ="<<phMax_F[jet]/jtpt<<std::endl;	      
	    //  std::cout<<"# tracks     = " <<trkN_I[jet]<<std::endl;
	    //  std::cout<<"       track frac. ="<<trkSum_F[jet]/jtpt<<std::endl;
	    //  std::cout<<"   MAX track frac. ="<<trkMax_F[jet]/jtpt<<std::endl;	    	      
	    //  assert(false);
	    //}
	    ////TEMP DO NOT LEAVE ME IN THERE UNCOMMENTED//


	    hJetQA_jttheta[theYBin][jtid] ->Fill(jttheta,weight_eS);
	    hJetQA_jtpz[theYBin][jtid]    ->Fill(jtpz,weight_eS);
	    hJetQA_jtp[theYBin][jtid]     ->Fill(jtp,weight_eS);
	    hJetQA_jtE[theYBin][jtid]     ->Fill(jtE,weight_eS);
	    hJetQA_jtEt[theYBin][jtid]    ->Fill(jtEt,weight_eS);
	    hJetQA_jtm[theYBin][jtid]     ->Fill(jtm,weight_eS);
	    hJetQA_jtmt[theYBin][jtid]    ->Fill(jtmt,weight_eS);
	    hJetQA_jty[theYBin][jtid]     ->Fill(jty,weight_eS);
	    if(jtid==1)hJetQA_jteta_jty          ->Fill(absreceta,absjty,weight_eS);
	  
	    
	    //jets	
	    hJetQA_jtptEntries[theEtaBin][jtid]->Fill(jtpt,1.);	  	
	    hJetQA[theEtaBin][ind][jtid]->Fill(jtpt, weight_eS); ind++;
	    hJetQA[theEtaBin][ind][jtid]->Fill(rawpt_F[jet], weight_eS); ind++;	    	    
	    hJetQA[theEtaBin][ind][jtid]->Fill(eta_F[jet], weight_eS); ind++;
	    hJetQA[theEtaBin][ind][jtid]->Fill(phi_F[jet], weight_eS); ind++;
	    
	    //tracks
	    hJetQA[theEtaBin][ind][jtid]->Fill(trkN_I[jet], weight_eS); ind++;
	    hJetQA[theEtaBin][ind][jtid]->Fill(trkSum_F[jet]/jetIDpt, weight_eS); ind++;
	    hJetQA[theEtaBin][ind][jtid]->Fill(trkMax_F[jet]/jetIDpt, weight_eS); ind++;
	    hJetQA[theEtaBin][ind][jtid]->Fill(trkHardN_I[jet], weight_eS); ind++;
	    hJetQA[theEtaBin][ind][jtid]->Fill(trkHardSum_F[jet]/jetIDpt, weight_eS); ind++;
	    
	    //PF photons
	    hJetQA[theEtaBin][ind][jtid]->Fill(phN_I[jet], weight_eS); ind++;       //HIN PHSUM/JETIDPT ~= SMP NEUTRALEMFRAC
	    hJetQA[theEtaBin][ind][jtid]->Fill(phSum_F[jet]/jetIDpt, weight_eS); ind++;
	    hJetQA[theEtaBin][ind][jtid]->Fill(phMax_F[jet]/jetIDpt, weight_eS); ind++;
	    hJetQA[theEtaBin][ind][jtid]->Fill(phHardN_I[jet], weight_eS); ind++;
	    hJetQA[theEtaBin][ind][jtid]->Fill(phHardSum_F[jet]/jetIDpt, weight_eS); ind++;
	    
	    //PF charged hadrons
	    hJetQA[theEtaBin][ind][jtid]->Fill(chN_I[jet], weight_eS); ind++;
	    hJetQA[theEtaBin][ind][jtid]->Fill(chSum_F[jet]/jetIDpt, weight_eS); ind++;//HIN CHSUM/JETIDPT ~= SMP CHHADFRAC
	    
	    hJetQA[theEtaBin][ind][jtid]->Fill(chMax_F[jet]/jetIDpt, weight_eS); ind++;
	    hJetQA[theEtaBin][ind][jtid]->Fill(chHardN_I[jet], weight_eS); ind++;
	    hJetQA[theEtaBin][ind][jtid]->Fill(chHardSum_F[jet]/jetIDpt, weight_eS); ind++;
	    
	    //PF neutral hadons
	    hJetQA[theEtaBin][ind][jtid]->Fill(neN_I[jet], weight_eS); ind++;   
	    hJetQA[theEtaBin][ind][jtid]->Fill(neSum_F[jet]/jetIDpt, weight_eS); ind++; //HIN NESUM/JETIDPT ~= SMP NEUTRALHADANDHFFRAC
	    hJetQA[theEtaBin][ind][jtid]->Fill(neMax_F[jet]/jetIDpt, weight_eS); ind++;
	    
	    //PF electrons
	    hJetQA[theEtaBin][ind][jtid]->Fill(eN_I[jet], weight_eS); ind++;   
	    hJetQA[theEtaBin][ind][jtid]->Fill(eSum_F[jet]/jetIDpt, weight_eS); ind++; //HIN ESUM/JETIDPT ~= SMP CHEMFRAC
	    hJetQA[theEtaBin][ind][jtid]->Fill(eMax_F[jet]/jetIDpt, weight_eS); ind++;
	    
	    //PF muons
	    hJetQA[theEtaBin][ind][jtid]->Fill(muN_I[jet], weight_eS); ind++;   
	    hJetQA[theEtaBin][ind][jtid]->Fill(muSum_F[jet]/jetIDpt, weight_eS); ind++;
	    hJetQA[theEtaBin][ind][jtid]->Fill(muMax_F[jet]/jetIDpt, weight_eS); ind++;
	    
	    //PF particle sums
	    hJetQA[theEtaBin][ind][jtid]->Fill(neuMult, weight_eS); ind++;	   //HIN NEUMULT ~= SMP NEUMULT 
	    hJetQA[theEtaBin][ind][jtid]->Fill(chMult, weight_eS); ind++; //HIN CHMULT ~= SMP CHMULT 
	    hJetQA[theEtaBin][ind][jtid]->Fill(numConst, weight_eS); ind++; //HIN NUMCONST ~= SMP CONSTCNT	      			      
		...
		int ind=dijtInd;
		hJetQA[theEtaBin][ind][jtid]->Fill( A_j , weight_eS ); ind++; 
		hJetQA[theEtaBin][ind][jtid]->Fill( x_j , weight_eS ); ind++;       
		hJetQA[theEtaBin][ind][jtid]->Fill( dphi , weight_eS ); ind++;       
		hJetQA[theEtaBin][ind][jtid]->Fill( firstGoodJetPt , weight_eS ); ind++;       
		hJetQA[theEtaBin][ind][jtid]->Fill( secondGoodJetPt , weight_eS ); ind++; }
	...
	for(int jtid=0;jtid<2;jtid++){	  
	  ...
	  if(fillDataJetTrigQAHists){       	
	    ...
	    //no exclusion between triggers
	    if(MBtrgDec)  hpp_incHLTMB[theEtaBin][jtid]->Fill(jtpt, (double)MBtrgPscl); //minbias
	    if(trgDec[0]&&!(trgPt<HLTthresh[0]))  hpp_incHLT40[theEtaBin][jtid]->Fill(jtpt, (double)trgPscl[0]);
	    if(trgDec[1]&&!(trgPt<HLTthresh[1]))  hpp_incHLT60[theEtaBin][jtid]->Fill(jtpt, (double)trgPscl[1]);
	    if(trgDec[2]&&!(trgPt<HLTthresh[2]))  hpp_incHLT80[theEtaBin][jtid]->Fill(jtpt, (double)trgPscl[2]);
	    if(trgDec[3]&&!(trgPt<HLTthresh[3]))  hpp_incHLT100[theEtaBin][jtid]->Fill(jtpt, (double)trgPscl[3]);  
	    
	    //with exclusion
	    if(isMB)   {
	      hpp_excHLTMB[theEtaBin][jtid]->Fill( jtpt , weight_eS);	//minbias
	    }	    
	    else if(is40 )  hpp_excHLT40[theEtaBin][jtid]->Fill( jtpt , weight_eS);
	    else if(is60 )  hpp_excHLT60[theEtaBin][jtid]->Fill( jtpt , weight_eS);
	    else if(is80 )  hpp_excHLT80[theEtaBin][jtid]->Fill( jtpt , weight_eS);
	    else if(is100)  hpp_excHLT100[theEtaBin][jtid]->Fill( jtpt , weight_eS); 
	  ...
	  if(fillDataTupelJetQAHists){
	    ...
	    int varind=0;
	    hTupelJetQA[theEtaBin][varind][jtid]->Fill(jetPt                  ->at(jet),weight_eS);varind++;
	    hTupelJetQA[theEtaBin][varind][jtid]->Fill(jetEta                 ->at(jet),weight_eS);varind++;
	    hTupelJetQA[theEtaBin][varind][jtid]->Fill(jetPhi                 ->at(jet),weight_eS);varind++;
	    hTupelJetQA[theEtaBin][varind][jtid]->Fill(jetE                   ->at(jet),weight_eS);varind++;
	    hTupelJetQA[theEtaBin][varind][jtid]->Fill(jetID                  ->at(jet),weight_eS);varind++;
	    hTupelJetQA[theEtaBin][varind][jtid]->Fill(jetRawPt               ->at(jet),weight_eS);varind++;
	    hTupelJetQA[theEtaBin][varind][jtid]->Fill(jetRawE                ->at(jet),weight_eS);varind++;
	    hTupelJetQA[theEtaBin][varind][jtid]->Fill(jetHfHadE              ->at(jet),weight_eS);varind++;
	    hTupelJetQA[theEtaBin][varind][jtid]->Fill(jetHfEmE               ->at(jet),weight_eS);varind++;
	    hTupelJetQA[theEtaBin][varind][jtid]->Fill(jetChHadFrac           ->at(jet),weight_eS);varind++;
	    hTupelJetQA[theEtaBin][varind][jtid]->Fill(jetNeutralHadAndHfFrac ->at(jet),weight_eS);varind++;
	    hTupelJetQA[theEtaBin][varind][jtid]->Fill(jetChEmFrac            ->at(jet),weight_eS);varind++;
	    hTupelJetQA[theEtaBin][varind][jtid]->Fill(jetNeutralEmFrac       ->at(jet),weight_eS);varind++;
	    hTupelJetQA[theEtaBin][varind][jtid]->Fill(jetChMult              ->at(jet),weight_eS);varind++;
	    hTupelJetQA[theEtaBin][varind][jtid]->Fill(jetConstCnt            ->at(jet),weight_eS);varind++;
	    hTupelJetQA[theEtaBin][varind][jtid]->Fill(jetNeuMult                      ,weight_eS);varind++;
	...
	if(etabin_bool_helper[i]){//don't undergo expensive covmatrix calc if the etabin wasn't filled.
	  ...
	  fillCovMatrix( (TH2D*)hpp_covmat_eta_arr[i], (TH1D*)hpp_covmat_eta_arr_helpers[i],  (double) weight_eS);
	...
	for(int i=0; i<nbins_abseta; i++){
	  if(JEC_etabin_bool_helper[i]){//don't undergo expensive covmatrix calc if the etabin wasn't filled.
	    ...
	    fillCovMatrix( (TH2D*)hpp_covmat_eta_arr_JEC_sysup[i], (TH1D*)hpp_covmat_eta_arr_helpers_JEC_sysup[i], (double) weight_eS);
	    ...
	    fillCovMatrix( (TH2D*)hpp_covmat_eta_arr_JEC_sysdown[i], (TH1D*)hpp_covmat_eta_arr_helpers_JEC_sysdown[i],  (double) weight_eS);
	...
	if(ybin_bool_helper[i]){//don't undergo expensive covmatrix calc if the etabin wasn't filled.
	  //fillCovMatrix( (TH2D*)hpp_covmat_y_arr[i], (TH1D*)hpp_covmat_y_arr_helpers[i], nbins_pt , (double) weight_eS);
	  fillCovMatrix( (TH2D*)hpp_covmat_y_arr[i], (TH1D*)hpp_covmat_y_arr_helpers[i],  (double) weight_eS);
	  hpp_covmat_y_arr_helpers[i]->Reset();//reset contents of TH1 without resetting binning, min/max, etc....	  
	}//end normal etabin hlelper	  
	ybin_bool_helper[i]=false;	
      ...
      if(fillDataJetJECUncHists){
	for(int i=0; i<nbins_absy; i++){
	  if(JEC_ybin_bool_helper[i]){//don't undergo expensive covmatrix calc if the etabin wasn't filled.
	    //fillCovMatrix( (TH2D*)hpp_covmat_y_arr_JEC_sysup[i], (TH1D*)hpp_covmat_y_arr_helpers_JEC_sysup[i], nbins_pt , (double) weight_eS);
	    fillCovMatrix( (TH2D*)hpp_covmat_y_arr_JEC_sysup[i], (TH1D*)hpp_covmat_y_arr_helpers_JEC_sysup[i],  (double) weight_eS);
	    hpp_covmat_y_arr_helpers_JEC_sysup[i]->Reset();//reset contents of TH1 without resetting binning, min/max, etc....	  	    
	    //fillCovMatrix( (TH2D*)hpp_covmat_y_arr_JEC_sysdown[i], (TH1D*)hpp_covmat_y_arr_helpers_JEC_sysdown[i], nbins_pt , (double) weight_eS);
	    fillCovMatrix( (TH2D*)hpp_covmat_y_arr_JEC_sysdown[i], (TH1D*)hpp_covmat_y_arr_helpers_JEC_sysdown[i],  (double) weight_eS);
	    hpp_covmat_y_arr_helpers_JEC_sysdown[i]->Reset();//reset contents of TH1 without resetting binning, min/max, etc....	  	    
	  }//end JECUnc specific etabin helper
	  JEC_ybin_bool_helper[i]=false;
	}//end JECUnc specific etabin loop	      
      }//end JECUnc specific covmatrix
  ...
  if(filelistIsJet80)    std::cout<<"--------------------- HighPtJet80 ----------------------" <<std::endl;
  if(filelistIsLowerJets)std::cout<<"------------------- HighPtLowerJets --------------------" <<std::endl;
  ...
  std::cout<<std::endl<<"----------------- TRIGGER EVENT COUNTS -----------------"<<std::endl;  
  if(filelistIsJet80){
    std::cout<<"# events passing L1J48  [inclusive] = " << h_NEvents_trigd_incl_L1J48->GetBinContent(1)   << std::endl; 
    std::cout<<"# events passing HLT80  [inclusive] = " << h_NEvents_trigd_incl_HLT80->GetBinContent(1)   << std::endl; 
    std::cout<<"# events passing HLT80  [EXCLUSIVE] = " << h_NEvents_trigd_excl_HLT80->GetBinContent(1)   << std::endl; 
    if(useHLT100)std::cout<<"REMEMBER YOU NEED TO FIX ME IF YOU USE HLT100 EVER AGAIN!!!!"<<std::endl;
  }
  if(filelistIsLowerJets){
    std::cout<<"# events passing L1J40  [inclusive] = " << h_NEvents_trigd_incl_L1J40->GetBinContent(1)   << std::endl; 
    std::cout<<"# events passing HLT60  [inclusive] = " << h_NEvents_trigd_incl_HLT60->GetBinContent(1)   << std::endl; 
    std::cout<<"# events passing HLT60  [EXCLUSIVE] = " << h_NEvents_trigd_excl_HLT60->GetBinContent(1)   << std::endl;
    std::cout<<std::endl;
    std::cout<<"# events passing L1J28  [inclusive] = " << h_NEvents_trigd_incl_L1J28->GetBinContent(1)   << std::endl; 
    std::cout<<"# events passing HLT40  [inclusive] = " << h_NEvents_trigd_incl_HLT40->GetBinContent(1)   << std::endl; 
    std::cout<<"# events passing HLT40  [EXCLUSIVE] = " << h_NEvents_trigd_excl_HLT40->GetBinContent(1)   << std::endl; 
    std::cout<<std::endl;
    std::cout<<"# events passing (trg40||trg60) [inclusive] = " << h_NEvents_trigd_incl_HLT40or60->GetBinContent(1)   << std::endl; 
    std::cout<<"# events passing (trg40||trg60) [EXCLUSIVE] = " << h_NEvents_trigd_excl_HLT40or60->GetBinContent(1)   << std::endl; 
  }
