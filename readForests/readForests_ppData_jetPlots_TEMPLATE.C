...
int readForests_ppData_jetPlots( std::string inFilelist , int startfile , int endfile , 
  ...
  // section where you declare vertex histograms, make a switch you can turn on/off for debugging, put with other hist switches at top.
  // i suggest doing so in a similar way to hVz and hWVz, starting with a null pointer
  /*
    TH1F *hEvtPUHist1=NULL, hEvtPUHist2=NULL;
    if(fillDataEvtPUHists){
    hEvtPUHist1=new TH1F();
    hEvtPUHist2=new TH1F();
    }
  */
  ...
  if(fillDataJetTrigQAHists){
    ...
    if(doHLTInEffCheck) {
      hpp_HLT40InEff  = new TH1F( "isHLT40_inEff"  , "isHLT40  inEff Check", 200, 0,2000 ); 
	...
	else if(hJetTrigQATitleArray[k]=="hpp_HLT80")  hpp_TrgObj80[j]   = new TH1F(hTitle.c_str(), (hJetTrigQADescArray[j]).c_str(), 2000,0,2000);
	else if(hJetTrigQATitleArray[k]=="hpp_HLT100") hpp_TrgObj100[j]  = new TH1F(hTitle.c_str(), (hJetTrigQADescArray[j]).c_str(), 2000,0,2000);	
	else if(hJetTrigQATitleArray[k]=="hpp_HLTComb")     hpp_TrgObjComb[j] = new TH1F(hTitle.c_str(), (hJetTrigQADescArray[j]).c_str(), 2000,0,2000);
	else if(hJetTrigQATitleArray[k]=="hpp_TrgCombTest") hpp_CombJetpT[j]  = new TH1F(hTitle.c_str(), (hJetTrigQADescArray[j]).c_str(), 2000,0,2000);       
  ...
  // THIS IS A PILEUP VERTEX FILTER. OPEN AN EXAMPLE HIFOREST TO SEE WHICH OTHERS ARE AVAILABLE IN THE SKIM ANALYSIS TREE
  ...
  //you may want to friend the "ppTrack" tree+directory to the other trees. Tracks tend to be inseparable from vertex analysis
  //to do this, simply add the tree+directory name in the HiForest files (use a sample file you have locally) to the "dataTreeNames" or "MCTreeNames" arrays defined in readForests_jetPlots.h
  /*
    fakeClass someVariable;
    jetpp[8]->SetBranchAddress("someBranchNameinppTrack", &someVariable);
   */
  ...
  for(UInt_t nEvt = 0; nEvt < NEvents_read; ++nEvt) {
    ...
    //where you'll apply the pileup filter
    ...
    // fill evt vz histo
    //where you'll likely*** want to fill your event level pileup study histograms. May find with time your analysis could require a bit more sophistication (a loop over tracks, a loop over tracks within a jet, etc.)
    // generally speaking, your event-hists be filled with entries weighted with "weight_eS"
    if(fillDataEvtQAHists){
      hVz->Fill(vz_F, 1.0);    
    ...
    // TRACK LOOP
    // if you wanted to loop over tracks, use the JET LOOP as an example to set it up.
    // if you want to loop over tracks, job time will probably spike. Consider how much of the jetloop below you need for your jobs to be insightful (hint; dijet plots useless and take up a lot of time, set fillBasicJetPlotsOnly to true to be more efficient
